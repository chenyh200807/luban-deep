...
def test_answer_citation_audit_rejects_footer_rows_without_visible_body_markers(tmp_path) -> None:
        ...
        json.dumps(
            {
                ...
                "cases": [
                    {
                        "case_id": "footer_row_without_body_marker",
    ...
    assert result["results"][0]["failures"] == ["unknown_footer_rows:〔1〕"]
...
def test_answer_citation_audit_reports_negative_cases(tmp_path) -> None:
    ...
    assert result["footer_coverage"] == 0.5
    assert result["hidden_leak_count"] == 1
    assert by_case["missing_footer"]["failures"] == ["missing_footer"]
    assert by_case["orphan_body_marker"]["failures"] == ["missing_footer_rows:〔2〕", "unknown_footer_rows:〔1〕"]
    assert by_case["forbidden_term"]["failures"] == ["hidden_forbidden_terms"]
