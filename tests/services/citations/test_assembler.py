...
def test_assembles_structured_references_without_mutating_response() -> None:
...
def test_does_not_mark_markdown_headings_as_cited_claims() -> None:
    ...
    assert "楼地面应满足隔声、保温、防水、防火要求。" in cited.response
    assert "- 铺装面层应平整、防滑、耐磨、易清洁。" in cited.response
...
def test_preserves_legal_document_numbers_and_teaching_basis_lines() -> None:
    ...
    assert "〔1〕" not in cited.response.replace("〔2020〕", "")
    validate_cited_answer(cited)
