...
# --- Bug X: per-option explanation intent vs answer-value request (single source) ---


def test_wants_per_option_explanation_detects_elaboration_requests() -> None:
    from deeptutor.services.question_followup import wants_per_option_explanation
    ...
    for message in (
        ...
        assert wants_per_option_explanation(message) is True, message


def test_wants_per_option_explanation_excludes_answer_value_requests() -> None:
    from deeptutor.services.question_followup import wants_per_option_explanation
    ...
    for message in (
        ...
        assert wants_per_option_explanation(message) is False, message


def test_wants_per_option_explanation_detects_per_item_phrasings() -> None:
    """Hardening 4 (Codex P2): 逐项话术也应判 wants_per_option_explanation。"""
    from deeptutor.services.question_followup import wants_per_option_explanation
...
def test_wants_per_option_explanation_per_item_excludes_answer_value() -> None:
    """逐项 markers 不能误伤纯答案值请求。"""
    from deeptutor.services.question_followup import wants_per_option_explanation
