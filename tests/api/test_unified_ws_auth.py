...
def test_ws_start_turn_normalizes_legacy_interaction_fields_into_interaction_hints(
    ...
    assert "subject_domain" not in config
    assert "teaching_mode" not in config
    assert config["interaction_hints"]["product_surface"] == "prelaunch_audit"
    assert config["interaction_hints"]["entry_role"] == "tutorbot"
    assert config["interaction_hints"]["subject_domain"] == "construction_exam"
    assert config["interaction_hints"]["teaching_mode"] == "smart"
