...
def _fake_patch_targets() -> list[tuple[Any, str]]:
    ...
    return [
        (secure_router_mod, "resolve_auth_context"),
        (secure_router_mod, "enforce_websocket_rate_limit"),
        (unified_ws_mod, "enforce_websocket_rate_limit"),
        (session_pkg, "get_turn_runtime_manager"),
        (llm_config_mod, "get_llm_config"),
        (context_builder_mod, "ContextBuilder"),
        (memory_mod, "get_memory_service"),
        (learner_state_mod, "get_learner_state_service"),
        (deep_question_mod, "write_grading_error_events"),
        (grader_mod, "SubmissionGraderAgent"),
        (orchestrator_mod.ChatOrchestrator, "_select_capability"),
        (runtime_shadow_adapter, "_build_best_quality_draft"),
    ]


@contextmanager
def _installed_ws_fakes(
    runtime: TurnRuntimeManager,
    *,
    user_id: str,
    write_calls: list[dict[str, Any]] | None = None,
    engine_calls: list[dict[str, Any]] | None = None,
):
    targets = _fake_patch_targets()
    snapshot = [(target, attr, getattr(target, attr)) for target, attr in targets]
    try:
        ws._install_fakes(
            runtime,
            user_id=user_id,
            write_calls=write_calls if write_calls is not None else [],
            engine_calls=engine_calls if engine_calls is not None else [],
        )
        secure_router_mod.resolve_auth_context = lambda _a: ws._auth_ctx(user_id)
        yield
    finally:
        for target, attr, value in reversed(snapshot):
...
def _client():
    ...
    with _installed_ws_fakes(runtime, user_id=_CUR["user"]):
...
def test_ws_pgo_shadow_readback_consumes_real_hash_pinned_knowql_query(
    ...
    blob = str(metadata)
    assert "official_slice" not in blob
    assert "knowledge_point" not in str(shadow.get("runtime_points", []))
    assert "answer_key_authority" not in blob
...
def test_ws_pgo_same_attempt_grading_to_brain_readback(monkeypatch, tmp_path) -> None:
    ...
    with _installed_ws_fakes(runtime, user_id=user_id):
        monkeypatch.setattr(learner_state_mod, "get_learner_state_service", lambda: service)
        with TestClient(ws._build_ws_app()) as client:
