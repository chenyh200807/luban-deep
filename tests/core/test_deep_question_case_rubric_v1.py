...
def test_pgo_bank_slot_canary_uses_authenticated_qa_operator_only(monkeypatch: pytest.MonkeyPatch) -> None:
    from deeptutor.capabilities.deep_question import _case_rubric_bank_slot_for_context

    monkeypatch.setenv("LUBAN_CASE_RUBRIC_BANK_SLOT_CANARY_ENABLED", "true")
    qa_context = UnifiedContext(
        user_message="m",
        metadata={
            "authenticated_user_id": "qa_stage5_user",
            "billing_context": {"user_id": "real_user_attempted_override"},
        },
    )
    operator_context = UnifiedContext(
        user_message="m",
        metadata={"authenticated_user_id": "operator_stage5_user"},
    )
    test_context = UnifiedContext(
        user_message="m",
        metadata={"authenticated_user_id": "test_stage5_user"},
    )
    spoofed_context = UnifiedContext(
        user_message="m",
        metadata={
            "billing_context": {"user_id": "qa_spoofed"},
        },
        config_overrides={"user_id": "operator_spoofed"},
    )

    assert _case_rubric_bank_slot_for_context(qa_context) == "pgo"
    assert _case_rubric_bank_slot_for_context(operator_context) == "pgo"
    assert _case_rubric_bank_slot_for_context(test_context) == "legacy"
    assert _case_rubric_bank_slot_for_context(spoofed_context) == "legacy"
...
@pytest.mark.asyncio
async def test_case_rubric_v1_stage5_canary_passes_pgo_slot_for_authenticated_qa(
    ...
    def _fake_load_rubric(qid: str, *, slot: str | None = None) -> list[dict[str, Any]]:
        ...
        return pgo_rubric if qid == "case-9006" and slot == "pgo" else []
    ...
    monkeypatch.setattr(G, "batch_judge_async", _fake_batch_async)
    context = _case_context(user_id="qa_stage5_user", rubric_v1=False)
    context.metadata["authenticated_user_id"] = "qa_stage5_user"

    result = await _run_case(monkeypatch, context)

    event = result["luban_case_rubric_v1"]["grading_event"]
    assert captured_slots == ["pgo"]
