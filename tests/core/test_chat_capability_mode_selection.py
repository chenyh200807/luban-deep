...
async def test_chat_capability_fast_mode_ignores_web_search(
    ...
    class FakePipeline:
        ...
        async def run(self, context: UnifiedContext, bus: StreamBus) -> None:
            ...
            captured["enabled_tools"] = list(context.enabled_tools)
            captured["knowledge_bases"] = list(context.knowledge_bases)
            await bus.result({"response": "ok", "chat_mode": "fast"}, source="chat")
...
def test_agentic_chat_pipeline_fast_mode_filters_to_rag(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "deeptutor.agents.chat.agentic_pipeline.get_llm_config",
...
@pytest.mark.asyncio
async def test_agentic_chat_pipeline_deep_mode_skips_independent_observing_stage(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "deeptutor.agents.chat.agentic_pipeline.get_llm_config",
    ...
    async def _unexpected_observing(*_args, **_kwargs):
        raise AssertionError("deep mode should not run a separate observing llm stage")
...
@pytest.mark.asyncio
async def test_agentic_chat_pipeline_ignores_legacy_deep_hints_without_canonical_chat_mode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "deeptutor.agents.chat.agentic_pipeline.get_llm_config",
    ...
    monkeypatch.setattr(pipeline, "_should_try_retrieval_first", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(pipeline, "_stage_thinking", AsyncMock(return_value="需要先拆解问题。"))
    observing = AsyncMock(return_value=("这里是观察结果。", {"label": "Observing"}))
    monkeypatch.setattr(pipeline, "_stage_observing", observing)
    monkeypatch.setattr(pipeline, "_stage_acting", AsyncMock(return_value=[]))
    monkeypatch.setattr(
        pipeline,
        "_stage_responding",
        AsyncMock(return_value=("这是最终回答。", {"label": "Final response"})),
    )
    ...
    assert observing.await_count == 1
    assert pipeline._stage_thinking.await_count == 1
    assert pipeline._stage_acting.await_count == 1
    assert pipeline._stage_responding.await_count == 1
