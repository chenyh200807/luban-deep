...
32b. "学员本轮粘贴的题是否=题库某道原题"是 **identity 判断而非 relevance 判断**。text/vector/option-overlap 只供给候选，所有题型都必须通过唯一的 `exact_question_identity_corresponds` 才能铸成 `question_exact_*` authority。case/case_study 的身份面必须包含题库 `background_context + stem`；仅有相似度、query shape、同一通用小问或 `case_bundle` 覆盖都没有放行权。case 不允许以学员截断文本被题库完整案例包含为由晋升；无法证明完整背景同题时降级普通 `questions_bank`。MCQ 可以用选项扩宽证据面，但题干仍须独立覆盖；计算题还必须满足数值、单位、角色与求解目标不变量。所有模糊路径都必须保留真实分数，不得人为抬置信；失败时回普通 RAG/主 LLM。真原题的口语前后缀、换行、全半角及少量 OCR 噪声可以由归一化与 ≥0.90 字符覆盖容忍，但共享域词或相似模板永远不是 identity。
