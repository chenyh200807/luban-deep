...
44. `retrieval_profile` 是**检索深度声明**（L1 瘦身检索，2026-08-01），不是第二条检索入口、不是路由 authority、不是 TutorBot mode：调用方声明「这一轮我消费什么」，统一 pipeline 在**同一条管线内**按 profile 短路，不得为某个 profile 分叉出平行检索函数。命名单一权威 = `deeptutor/services/rag/retrieval_profiles.py`。
    ...
    - `unanchored_exam_query`（2026-08-11，低信息真题查询锁权轮的题面供给收口，live 防冒充钉 3/3 红实证；同日 16-agent 复审 F1-F8 收权修订）：本轮 `exact_question_blocked_reason=low_information_exam_query`（学员指代的题无法锚定，exact 题目权威被 lifecycle gate 拒绝武装）时，**题目面材料不得进入模型上下文**。pipeline 整轮不武装两条题目面通道——`questions_bank` 检索族（`search_questions_bank=False` 连锁熄灭 exact 文本探针、bank 向量、`question_exact_vector` 派生、`case_like` second pass 与 `exact_question` payload）与 exam 卷面 chunk（`search_exam_chunks=False`）；textbook/standard 通道**照常**（模型仍有讲解依据，这是供给收权不是拒答降级），降级判据不变（`retrieval_warnings` 单一权威；本 profile 下 empty-index 检测**保持全量轮语义**，与 `case_grading_identity` 的"空即正常"不同）。
        - **决策权威唯一且 per-turn**（复审 F3/F4）：`retrieval_profiles.resolve_turn_retrieval_profile`（纯函数）是本 profile 的唯一推导点，只吃调用点闭包里的本轮 runtime_metadata——**禁止**从共享可变 tool state（`RAGAdapterTool._runtime_context`，并发轮 `_set_tool_context` 会覆盖）推导；**服务端推导压过一切调用方声明**（含 `case_grading_identity`），模型自发 rag 调用 args 里未在工具 schema 声明的 `retrieval_profile` 一律剥掉（`AgentLoop._prepare_rag_tool_args`）。盖章点 = loop 侧 prefetch / in-loop dispatch / exact-fast-path 三处（机械位，零决策）。
        - **service 层 provider 无关执法**（复审 F2/F7）：`RAGService` 是第二道收口——①锁权轮跳过 historical-question 回注（`_apply_historical_question_context` / `_build_historical_question_result`，二者会整段回注『【题库原题】…标准答案…解析…』）；②provider 结果**必须回声** `retrieval_profile`，不回声（llamaindex/kbv5 经 `**kwargs` 静默吞声明）即 fail-closed 到零供给空结果（`retrieval_status=question_surface_disarm_unsupported`，不标 degraded），宁空不冒充。
        - **观测诚实**（复审 F8）：锁权轮导出的 `retrieval_plan` 必须反映 disarm 后的真实 fanout——`disable_question_surface_groups`（`retrieval_plan.py`，题目面组唯一名单 = `QUESTION_SURFACE_SOURCE_GROUPS`）把 `questions_bank`/`exam` 组标 disabled，trace 不得声称查了题库。
        - 调用点与 sink 不得再各自遮蔽/改写题库文本充当第二权威（sink 面 redact 已被 live 证伪：prefetch 先行注入，in-loop sink 永不通电）。域测试：`tests/services/rag/test_unanchored_exam_query_profile.py`（管线 + service 执法 + plan 诚实）、`tests/tutorbot/test_low_information_bank_disarm.py`（决策权威 + 盖章点 + loop 注入面 + per-turn 纪律）。
