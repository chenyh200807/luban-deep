...
43. 题型一致性 · cross-type case evidence 隔离(#23 第二层):当学生 query 不是案例题（`classify_query_shape` != `case_like`）时，`questions_bank` 里的案例题（`question_type` 含 case/案例）不得作为本轮判分 evidence——它们是别的题，其解析含别题的具体背景数值（如“中标价 1.7 亿”），判分 LLM 会锚定误用于本题（纯 prompt grounding 已 live 证伪、压不住 evidence 邻近数字锚定）。`SupabasePipeline._drop_cross_type_case_exam_evidence` 在 rerank 后确定性剔除；规则/标准由 standard / textbook / kb_chunks 提供，不受影响；剔空时返回空安全集（judge 退化为纯题面判分），绝不退回原结果（退回原=别题数值重新进入 evidence=过滤失效）。这是 exact-question 题型门（约束 8/42a，`_augment_case_exact_question_with_query` 撤销 case_study 命中）的召回侧延伸：第一层撤销“确定性拼别题标准作答”，本条隔离“别题案例 content 污染 judge evidence”。
