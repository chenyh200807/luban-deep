...
14. mobile assessment TestSet HTTP endpoints (`/api/v1/assessment/create`, `/api/v1/assessment/{quiz_id}`, `/api/v1/assessment/{quiz_id}/submit`, `/api/v1/assessment/{quiz_id}/report`) are non-chat, non-streaming adapters. They must not create a new WebSocket route, must not write turn/session state, and must keep hidden answer/grading artifacts server-side until deferred-feedback submit.
