...
- Stage 5 PGO bank-slot canary 只能由服务端环境开启：`LUBAN_CASE_RUBRIC_BANK_SLOT_CANARY_ENABLED=true` 时，V1 案例评分主链只允许服务端认证 `authenticated_user_id` 命中 `LUBAN_CASE_RUBRIC_BANK_SLOT_CANARY_COHORT`（默认 `qa_,operator_`）的请求显式读取 `pgo` slot；未命中 cohort 的请求必须显式读取 `legacy` slot，不能依赖进程默认值。客户端 config、billing_context、display user id、TutorBot hint 不得自授权 PGO 主链路。该 canary 不改变 `/api/v1/ws` transport、不授予 official/canonical 写权，`LUBAN_CASE_RUBRIC_V1_ENABLED=false` 仍是全局 kill switch；broad default flip 仍只能走 `LUBAN_CASE_RUBRIC_BANK_SLOT=pgo` + worker restart 的单独授权门。
