...
25. **跨 worker live 订阅以共享 SQLite store 为单一事件权威（强化 H15/H16；INFRA-1，2026-06-22 live eval 实证）**：`UVICORN_WORKERS>1` 时一个 turn 常运行在**兄弟 worker**，因此不在本 worker 的进程内 `turn_runtime._executions` 里。`subscribe_turn` 在 `execution is None` 且 turn 仍 `running` 时，**必须轮询共享 store 把事件 tail 到 terminal（`done`/`error` 或 store 状态转终态）**，不得只回放 backlog 就 return（旧行为=WS 订阅者收到 backlog 的 `session`/`progress` 后再无 live、无 terminal `done`——跨 worker 流式死，学生军团 eval `ttft=null` 占多数即此），也**不得对兄弟 worker 正在运行的 turn 调 `_recover_orphaned_running_turns` 标记 `failed`**（旧行为=跨 worker 主动破坏在跑的 turn）；只有 tail 期间持续无新事件超 `_CROSS_WORKER_TAIL_ORPHAN_TIMEOUT_SECONDS`（疑似 worker 崩溃的真孤儿）才回落 orphan recovery。进程内 `_executions` / live 队列只是同 worker 快路径，不是"turn 是否存活"的权威；catch-up 只服务同 worker 内存桥（`execution is not None`）。回归见 `tests/api/test_unified_ws_turn_runtime.py::test_subscribe_turn_tails_shared_store_when_turn_runs_on_sibling_worker`。
