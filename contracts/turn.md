...
- `turn_runtime` 可以把 question-domain action 适配成 `turn_semantic_decision`，但决策形状必须复用 `semantic_router.build_turn_semantic_decision` 的 canonical schema；不得在 runtime 内维护第二套 relation / next_action / allowed_patch 枚举或结果结构。首次练题生成即使尚无 `active_object`，也必须由该适配层产出 `next_action=route_to_generation` / `allowed_patch=set_active_object` 的 canonical decision；`deep_question` 只能消费这份决策，不得补造第二套 fallback truth。
