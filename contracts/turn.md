...
15. `/api/v1/ws` 传输边界设两道内存防护，二者只防资源放大、不改变 turn 状态机或事件 `seq` 语义：(a) 单条入站文本帧硬上限 `unified_ws._MAX_WS_INBOUND_FRAME_CHARS`（128K 字符），超限帧在 `json.loads` 前 fail-fast 拒绝、不进入 turn 链路；(b) 每个 live subscriber 的内存事件队列有界（`turn_runtime._MAX_LIVE_SUBSCRIBER_QUEUE_SIZE`），满载时丢弃**最旧** live 事件以封顶慢消费者内存——SQLite 始终是事件真相，客户端通过 `subscribe_turn` / `resume_from` 的 `after_seq` backlog 重放补齐被丢弃事件，且 terminal `done` / `None` 关闭哨兵始终送达（慢消费者不会因哨兵被丢而挂起）。不得用"放大 live 队列"替代 SQLite replay 作为送达保证。事件持久化时点（Battle1 W2-T3）：每个事件在任何 fan-out 之前已由 per-turn 内存分配器盖章 durable `seq`；非 delta 事件（result/error/done/session/tool_*/stage_*/progress 等一切客户端可依赖事件）持久化先于投递，且其落库事务包含之前全部缓冲的 delta；content 与 thinking delta（thinking=LLM 内部草稿流，非用户可见，owner 2026-07-12 裁决纳入缓冲通道）在 ≤250ms / 64 事件 / 2048 字符窗口内批量持久化，**或直到下一事件到达**（flush 由下一事件触发，无后台 flusher；同 worker 新订阅者 attach 时先 flush 再做 catchup 读）。窗口内进程 crash 最多丢失未 flush 的 content/thinking delta，终态事件零丢失、`seq` 连续前缀与 `after_seq` 重放语义不受影响。禁止把多个 delta 合并成一行持久化（会破坏 resume(after_seq) 逐事件等价）。旧小程序包的 `/api/v1/mobile/chat/start-turn` 与 `/api/v1/mobile/chat/start` 只允许作为隐藏 schema 的兼容 bootstrap alias，必须复用同一个 handler、同一限流和同一 `/api/v1/ws` stream bootstrap，不得定义第二套 turn/session/stream truth。
...
25. Hosted Luban 教学卡不得新增 WebSocket 路由或静态 answer 协议。其唯一例外是同一 `/api/v1/ws` 的 `Sec-WebSocket-Protocol: luban-preview-v1, <opaque-ticket>` 握手：ticket 必须仅由已认证站点签发、SQLite hash 持久化、绑定真实 `user_id + pack_id + 已创建 turn_id` 并短期失效。该 scoped socket 仅允许 `subscribe_turn` 且精确等于绑定 `turn_id`（可用 `after_seq` 重连）；`start_turn`、`resume_from`、`subscribe_session`、`cancel_turn` 与任何其他 turn 一律拒绝。它复用同一 turn event / replay / redaction authority，不能成为 bearer、session authority、billing identity 或 learner-state writer。教学卡问答仍属于 Mini Program conversation，session source 必须沿用 `wx_miniprogram`；卡片身份只进入 `product_surface` / card context。历史误写为 `luban_teaching_card` 的 `luban-preview:*` session 由 `SQLiteSessionStore` 初始化迁移一次性归一，历史 reader 不维护第二套来源分支。教学卡的 `lesson_viewed` bridge 只能委托 `record_lesson_view_evidence` 这一既有 writer；卡内提问不计入掌握，五题掌握仍只由 `RetestWritebackService` 的服务端重判写入。
...
- 所有 terminal 路径（成功、typed failure、异常、取消、超时）必须经过同一 billing terminal observer。只有服务端判定 `chargeable_assistant_content=true` 的完整可交付回答可请求 wallet capture；其余路径必须记录 `released/non_chargeable_assistant_content` 且扣费为零。observer 与 metrics 必须 fail-open，不能反过来改变 turn terminal truth；wallet ledger 仍是 applied balance mutation 的唯一 authority。
