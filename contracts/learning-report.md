...
- **稳定边界**：
  ...
  - 轻练确认是错后加分项，非记忆闭环唯一兜底：confirm=`unavailable`+`reason=safe_confirm_unavailable` 且同投影 `due_validation` 步 status ∈ `{scheduled,current,completed}`（到期验证已在轨）时，`reason` 改发 `confirm_covered_by_due_validation`（status 仍 `unavailable`，不新增相位）；语义是本轮错题 fact 并入第 5 步到期验证卷兜底必考，而非"不可用"。到期验证不在轨（降级 `unavailable`）时保持 `safe_confirm_unavailable`，不得谎报已并入。`confirm_supply_projection_unavailable` 的降级溯源不并入本改写。客户端按此 reason 做纯呈现映射（并入验证/轻量态），不得据此推导任何供给或掌握语义。
