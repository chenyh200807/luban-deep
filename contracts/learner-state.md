...
- 单次扫描共享：一个实质轮次的门决策（`_summary_gate_decision`）与摘要 source 构建（`_build_summary_source`）必须复用**同一次** `learner_memory_events` 本地读，由 `refresh_from_turn` 在锁内、当前轮次落账后读取一次并传入两者，杜绝每轮两遍全文件线性扫描。这是纯性能收口——门决策结果与 source 内容逐字节不变；两个消费者的 `events=None` 回退保持独立自读语义。此共享不覆盖 `read_snapshot` 自身的账本读（另一相位的独立读，不在此收口范围）。`read_snapshot` 在自己的相位内只运行一次 seed pass，随后通过私有 raw reader 各自读取当下 profile / summary / progress / events projection；只收敛重复前置读，不复用旧业务值，不改变公开 reader 或字段 authority。
...
- learner-state 事件身份只认 `event_identity.canonical_event_id`：UUID 形态统一为小写 compact hex，opaque id 只裁剪首尾空白。事件生成、local/remote mapping、exact read/cache、`item_event_refs`、terminal id 与 `cycle_anchor` 绑定必须共用该内核，任何 consumer 不得自行选择 UUID 字符串格式或另造 alias。
- local/remote 副本按 canonical event id 经同一个 arbitration helper 合并；同一 canonical id 的不可变业务正文必须语义一致（`created_at` 比较同一 UTC instant，JSON 数值 round-trip 的 `1`/`1.0` 等价，但布尔值与数值不等价）。正文冲突时 list/exact reader 必须告警并丢弃该 id 的全部副本（fail-closed），不得用 local-first、remote-first、cache-first 或时间排序静默选赢家；exact 不得保留 process-local cache，一致副本折叠必须是只读、幂等且不得产生新 writeback。
- exact evidence read 不保留 process-local cache；每次读取都必须重新收集当前 WAL 与 durable store 副本再进入同一 arbitration helper，避免跨 worker 出现第二份事件状态。到期 probe 的 durable claim identity 是 `(user_id, probe_id, canonical cycle_anchor)`：Python writeback 与 Postgres RPC 共用 UUID compact-lower / opaque-trim 语义，RPC 必须先恢复最早历史等价 winner，再允许 canonical dedupe key 参与新请求并发仲裁。该 Postgres helper 只是 learner-state 内部 DB identity 实现，不新增跨模块 typed payload，因此不修改 schema registry。
