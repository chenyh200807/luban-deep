...
18. `mobile` adapter 的 billing / wallet 端点只能做认证身份、wallet identity、额度 fail-closed 和展示 read-model 装配；不得把钱包、旧会员流水或展示层状态升级为 capability routing / request config authority。旧会员流水只能通过显式迁移/对账开关参与展示，默认 authority 必须是 wallet ledger。计费是否生效由单一 flag `DEEPTUTOR_BILLING_ENFORCEMENT_ENABLED`（默认关闭，内测放行）控制；开启后 `/api/v1/chat/start-turn` 在 `start_turn` 之前做硬余额 fail-closed（见 `contracts/turn.md`），但该额度门只是 commerce 层 fail-closed，不得成为 capability route 或 request config authority。
