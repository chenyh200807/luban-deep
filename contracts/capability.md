...
29. `mobile` adapter 的 assessment TestSet 目录端点 (`/api/v1/assessment/topics`) 只能暴露 `assessment_forms` / blueprint coverage 派生的 read model，用于选择 topic TestSet。它不得触发 capability、不得启动 turn、不得根据 learner state 动态生成题目，也不得把 catalog status 升级为学习计划或 mastery authority。
