...
22. capability 产出的 grading / explanation 结构体（含 `MCQGradingResult.evidence_refs`、`CaseRubricItemResult.source_fields` 等）允许在服务端内部保留完整 hidden authority；但 `/api/v1/ws` 公开出库前必须经统一 redaction（详见 [contracts/turn.md](./turn.md) §硬约束 13）：evidence-style entry 中 `field` / `source_field` / `source_key` / `name` 指向 hidden authority 的整项 drop；`source_fields: list[str]` 中 hidden 元素过滤；不允许 capability 自行另起 transport-side rewriter 绕开统一 redactor。
