...
compat/projection/legacy writer became the operative source of a control-plane
fact in production.
...
``metadata["control_plane_shadow_hits"]``: a list of structured entries shaped::

    {
      ...
      "writer_role": "compat_projection" | "legacy_decider" | "production_decider",
      "writer_symbol": "<emitting symbol>",
      "path": "<code path tag>",
      "canonical_present": <bool>,
    }

Two gate numbers are derived from the window:

- ``legacy_production_decision_hits`` — hits whose ``writer_symbol`` is
  ``_select_legacy_capability`` AND whose ``path`` is ``disabled`` (the legacy
  capability decider drove a *production* routing decision while the semantic
  router was disabled / scope-excluded).
- ``compat_projection_production_reads`` — hits whose ``writer_role`` is a
  non-canonical writer (``compat_projection`` / ``production_decider``) AND
  whose ``canonical_present`` is ``False`` (a compat/fabricate writer became the
  operative source because the canonical decision was absent).

Semantics: each counted hit = one time a compat/fabricate/legacy writer became
the operative source in a real (non-synthetic, non-test_only) production turn.

Exit codes:
  0  both gate numbers are 0 over the window (gate green)
  1  any gate number > 0 (a compat/legacy writer was operative in production)
  2  fail-closed: the event log is unreadable, OR the window has zero turns
     carrying the instrumentation marker (no measured coverage). A window of
...
_COUNTING_METHOD = (
    ...
    "and is NOT-MEASURED (not 'clean'); each hit = one time a compat/fabricate/legacy "
    ...
    "window: no coverage means green cannot be claimed."
)
...
def _is_legacy_production_hit(hit: dict[str, Any]) -> bool:
    return (
        str(hit.get("writer_symbol") or "").strip() == _LEGACY_PRODUCTION_SYMBOL
        and str(hit.get("path") or "").strip() == _LEGACY_PRODUCTION_PATH
    )
...
def report_control_plane_shadow_hits(
    *,
    ...
    has_hits = legacy_hits > 0 or compat_reads > 0
    ...
    return {
        ...
        "legacy_production_decision_hits": legacy_hits,
