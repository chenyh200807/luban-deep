...
> **当前状态(2026-08-01)**:定榜跑 `promo_gate_20260801_r5_board` @ SHA `c53a5d8a`
> **24/24 PASS,零 infra 重试,OD 台账清零** —— 可宣传态达成(六病全关基线)。
> 报告副本:`docs/原始数据/数据盘点/2026-08-01-宣传门全量定榜r5.md`。
> 前一榜:`promo_gate_20260801_r4_board` @ `35ce8b22`,21/21 PASS。断言全部确定性机器可判,
...
(空——台账清零;r5 定榜 24/24 PASS @ `c53a5d8a`,零 infra 重试)

**待补场景(非产品病,是门的覆盖缺口)**

- **A10 断言当前无场景引用**:方案 C 消除了「兄弟行局部覆盖」形态后,
  `case_grading_partial_scope` 在整卷粘贴下不再出现,A10 实现保留但没有测例能触发。
  若以别的入口形态复现(例如只粘贴 2/4 小问的**残卷**),当前 24 场景抓不到。
  建议补一个 `t8b_partial_paper`(只粘贴 2/4 问 + 只答 1 问)把 A10 挂回去。
- **官方 rubric 通道未覆盖**:r5 全部案例场景 `grading_official_score_allowed=false`,
  门验的是诊断得分预估通道;R5 收入闸(官方成绩)仍在门外。
- **判分核 high 档偏松(观测项,非 FAIL)**:t9 实测 0.90 vs 金标 0.84(+0.06)。
  单点不下结论,连续几榜同向再立案。

### CLOSED

**OD-005 — 整卷全答被封顶误伤 / 半答假满分** — 门内常驻回归位,r5 起成对断言
`t8_group_bundle_half`(半答 1/4 问 ≤3.0/10)+ `t9_full_paper_full_answer`
(全答 ≥6.0/10)。r5 实测 2.5/10 与 9.0/10,`case_per_subq_grading=4/4`、
`case_subq_score_caps=q1:2.5,q2:2.5,q3:2.5,q4:2.5` 两处一致。
**单侧断言会漏判**:只有 t8 时,「一律压低」也能绿。
