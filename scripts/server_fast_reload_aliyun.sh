...
if [ -z "${existing_container_id}" ]; then
    echo "deeptutor 容器不存在，无法执行快速发布。请先完整部署。" >&2
    exit 1
fi
...
if [ -z "${container_id}" ]; then
    echo "deeptutor 容器未运行，无法完成快速发布。请先完整部署。" >&2
    exit 1
fi
...
适用范围: 已同步到 /root/deeptutor 的后端候选代码；本脚本重建 deeptutor 服务镜像并 force-recreate，不执行容器热补丁。
