...
_ASK_STATE_NEW = ('askAnswer:"", askRawResponse:"", askBlocks:[], askWorkflowEntries:[], '
                  'askWorkflowBadge:"", askWorkflowTitle:"", askWorkflowSub:"", '
...
_ASK_RENDER_VALS_S07_NEW = '''askOpen:this.state.askOpen, askText:this.state.askText, askLoading:this.state.askLoading,
      askBlocks:this.state.askBlocks, askWorkflowEntries:this.state.askWorkflowEntries, askWorkflowBadge:this.state.askWorkflowBadge, askWorkflowTitle:this.state.askWorkflowTitle, askWorkflowSub:this.state.askWorkflowSub, askError:this.state.askError, hasAnswer:!!this.state.askStreaming || !!this.state.askBlocks.length || !!this.state.askWorkflowActive,
...
def _inline_tutorbot_ask_method(pack_id: str) -> str:
    ...
    const renderWorkflow=function(active, extra){
      ...
      self.setState(Object.assign({askWorkflowEntries:workflowEntries,askWorkflowBadge:summary.badge||"",askWorkflowTitle:summary.headline||"",askWorkflowSub:summary.subline||"",askWorkflowActive:active!==false},extra||{}));
    };
    ...
    const connect=function(stream){
      ...
      try{
        ...
        socket.onclose=function(){
          if(completed)return;
          if(reconnects>=2){ fail("流式连接中断，请留在本页稍后重试。"); return; }
    ...
    this.setState({askLoading:true,askAnswer:"",askRawResponse:"",askBlocks:[],askWorkflowEntries:[],askWorkflowBadge:"",askWorkflowTitle:"",askWorkflowSub:"",askWorkflowActive:true,askStreaming:true,askError:""});
    renderWorkflow(true);
    try{
