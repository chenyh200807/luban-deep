...
- **练习防套路（选项设计硬规则）**：① **正确项位置打散**，不能恒在某一位（尤其别恒 A）——源数据正确项可写 index0，但在 `componentDidMount` 用固定 per-question 置换重排 `q.opts`（评分靠 `o.ok` 标志、不靠索引），整套分布要杂（如 C/D/B/A/C/D）；② **选项长度拉平**，正确项常是"最全采分句"天生最长，必须把干扰项写成等长完整句，让"选最长=正确"失效，至少一题的干扰项比正确项更长；③ 验收过一遍"只按位置 / 只按长度 能否全对"，能就回炉。（复盘 F02 v1：每题正确项恒 A 且恒最长，"选 A/选最长"即全对——两 tell 叠加、不用理解，是精品练习致命漏洞。）
...
不做纯文字框/流程框/判断框 · 不把 6+1 当标签 · 安全合同管理类不退回文字 · 不只整页淡入淡出 · **画面必须结合旁白（相位顺序+时机对齐字幕）** · **写 storyboard 前先过采分点覆盖矩阵、进 finished 前过双轴验收** · **对比类视觉两侧几何必须真不一样、画出唯一判别特征，不靠移标签假装** · **文字标签不压深色对象** · 不用 emoji · logo 用 mark-white/mark-red.png · 配音 老师 longanhuan_v3 / 学生 longlaotie_v3 · 改进若通用就回灌母版。
