...
6. 改两页互链文件名占位（讲解→练习、练习→讲解）；若因超 5 分钟分集，改名为 `<考点名>_讲解_上.dc.html`/`_下.dc.html`（三集加 `_中`，更多用 `_一/_二/_三…`），并加「上一集/下一集」互链——**分集命名仍按此规则，但位置照旧在项目根目录**。改完自检：三个卡都应出现在**编辑器 Pages 下拉里可切换**；若只出现在「All project files」，说明放错了子文件夹，移回根目录。
