...
### 4.3 时间轴控制区
红色圆形播放/暂停钮 + `range` 进度条（`accent-color:#cf4436`）+ 当前/总时长 + 全屏钮 ⛶。下面一排 beat 芯片（当前红底白字，其余 `#23282b`），末位「问答 Q&A」橙底。再下面一排功能按钮：「🤖 追问AI」红渐变 + 「📝 做练习 →」绿。
...
### 4.8 练习题卡（练习页）
进度条 + 题号/维度标签 → 题干 → **图示区**（每题必有：标尺/变化图/管道图/诊断图/答题纸）→ 选项（完整采分句，非关键词）→ 提交后每项展开解析 → 「📝 抄进答题纸的采分句」暗卡 → 行动钮 + 「🤖 这道题没搞懂？追问AI（已带题目）」。
