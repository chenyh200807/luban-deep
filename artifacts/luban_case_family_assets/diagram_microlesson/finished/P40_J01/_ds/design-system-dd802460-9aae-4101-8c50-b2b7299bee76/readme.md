...
不做纯文字框/流程框/判断框 · 不把 6+1 当标签 · 安全合同管理类不退回文字 · 不只整页淡入淡出 · **画面必须结合旁白（相位顺序+时机对齐字幕）** · **写 storyboard 前先过采分点覆盖矩阵、进 finished 前过双轴验收** · **对比类视觉两侧几何必须真不一样、画出唯一判别特征，不靠移标签假装** · **文字标签不压深色对象** · **不许为控制时长删减采分点，超 5 分钟必须按自然分段拆多集** · 不用 emoji · logo 用 mark-white/mark-red.png · 配音 老师 longanhuan_v3 / 学生 longlaotie_v3 · 改进若通用就回灌母版。
