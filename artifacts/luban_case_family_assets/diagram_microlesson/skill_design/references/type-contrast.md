...
- **现状**:✅ 草稿 `../C01_construction_joint_contrast.schema_draft.json`(施工缝留置;`contrast_pair_reveal_draft`,无 contrast renderer,schema 先行同 D01)。是 panel 加固的参考实现。可与 ⑥诊断共用错因/采分点资产;动 renderer 前先硬化:template_type 分派 + body 互斥 + student-safe 白名单。
