...
- **现状**:✅ 草稿 `../D01_answer_point_diagnosis.schema_draft.json`(无 renderer)。动 renderer 前先硬化:template_type 分派 + steps/diagnosis 互斥 + student_safe 白名单。采分点须 candidate→签发后才进生产判读。
