...
function applyMotion(activeEl,active,t){{if(!activeEl)return;const dur=Math.max(.001,active.end-active.start),p=clamp01((t-active.start)/dur),nodes=[...activeEl.querySelectorAll('[data-visible-node]')];const cameraPush=active.camera==='push-in'||active.camera==='spotlight'||active.camera==='answer-paper'||active.camera==='trace';stage.style.setProperty('--camera-scale',String(1+(cameraPush?0.035*ease(Math.min(p/.28,1)):0)));stage.style.setProperty('--camera-y',(active.camera==='pull-back'?String(-8*ease(p)):'0')+'px');nodes.forEach((node,i)=>{{const start=.04+i*.14,v=ease((p-start)/.18),name=node.dataset.visibleNode||'';node.style.opacity=String(v);node.style.transform=`translateY(${{(1-v)*10}}px) scale(${{0.96+v*0.04}})`;node.classList.toggle('node-focus',name===active.focus||name.includes(active.focus));}});if(active.id!==lastScene){{scenes.forEach(scene=>{{if(scene!==activeEl)scene.querySelectorAll('[data-visible-node]').forEach(node=>{{node.style.opacity='0';node.style.transform='translateY(10px) scale(.96)';node.classList.remove('node-focus');}});}});lastScene=active.id;}}}}
function paint(){{const t=Number(au.currentTime||scrubber.value||0);const active=sceneAt(t);const seg=segmentAt(t);lesson.classList.add('started');lesson.classList.toggle('paused',au.paused);lesson.classList.toggle('playing',!au.paused);let activeEl=null;scenes.forEach(el=>{{const on=el.dataset.sceneId===active.id;el.classList.toggle('active',on);if(on)activeEl=el;}});applyMotion(activeEl,active,t);chapters.forEach(el=>el.classList.toggle('on',t>=Number(el.dataset.t)&&Number(el.dataset.t)>=active.start-0.1));fill.style.width=(Math.min(t,DATA.totalSec)/DATA.totalSec*100)+'%';scrubber.value=String(Math.min(t,DATA.totalSec));cur.textContent=cur2.textContent=fmt(t);captionLine.textContent=seg.text||active.coach||'';captionLine.dataset.speaker=seg.speaker||'T';}}
