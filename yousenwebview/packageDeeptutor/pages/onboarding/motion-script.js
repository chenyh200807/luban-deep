...
// Fuse 式结构：wave(色浪) → hook(痛点轮播→品牌宣言) → p1/p2/p3(三页，光场逐页换色温)。
// p3 是终幕：自动播完触发 onFinish（色浪转深直落登录页）；手动接管时靠页内按钮出场。
// patch 的 key 是页 data 路径；全部幂等（手动跳幕整幕重放）。
"use strict";
...
module.exports = [
...

  {
    ...
    steps: [
      ...
      { at: 4600, patch: { "fx.ctaActions": true } },
    ],
  },
];
