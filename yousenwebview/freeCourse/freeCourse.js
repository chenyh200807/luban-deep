...
const promoImageCache = Object.create(null)
const promoImagePromiseMap = Object.create(null)
...
Component({
  ...
  data: {
    ...
    allmajor:{'一级建造师':{'建筑实务':63,'市政实务':61,'机电实务':60,'水利实务':491,'项目管理':58,'相关法规':62,'工程经济':59},
              ...
              '名师讲座':{'名师讲座':999},
              '教材精讲课':{'一建建筑实务':800,'一建机电实务':801,'一建水利实务':802,'一建项目管理':810,'一造案例分析':803,'一造水利案例':804,'一造水利计量':805,'一造交通案例':808,'一造交通计量':809,'一造土建计量':812,'咨询实务':806,'咨询评价':807,'监理三控':811},
            },
    multiArray: [['一级建造师', '二级建造师', '造价工程师', '监理工程师','咨询工程师','实操','名师讲座','教材精讲课'],
     ['建筑实务',
      '市政实务',
      '机电实务',
      '项目管理',
  ...
  methods: {
    initGratisCourseState: function() {
      ...
      this.modalImageSeq = 0;
    },
    preloadPromoImage: function(url) {
      if (!url) {
        return Promise.resolve('');
      }
      if (promoImageCache[url]) {
        return Promise.resolve(promoImageCache[url]);
      }
      if (promoImagePromiseMap[url]) {
        return promoImagePromiseMap[url];
      }
      promoImagePromiseMap[url] = new Promise(resolve => {
        wx.getImageInfo({
          src: url,
          success: res => {
            const nextPath = res && res.path ? res.path : url;
            promoImageCache[url] = nextPath;
            resolve(nextPath);
          },
          fail: () => {
            resolve('');
          ...
          complete: () => {
            delete promoImagePromiseMap[url];
          }
        });
      });
      return promoImagePromiseMap[url];
    ...
    syncGratisCourseModal: function(res) {
      ...
      const promoUrl = res.showimg || '';
      const promoTargetUrl = res.showurl || '';
      const modalSeq = ++this.modalImageSeq;
      if (this.modalDisplayTimer) {
        clearTimeout(this.modalDisplayTimer);
        this.modalDisplayTimer = null;
      }
      if (!promoUrl || modalLocked) {
      ...
      if (promoImageCache[promoUrl]) {
        this.setData({
          showModal: true,
          showimg: promoImageCache[promoUrl],
          showurl: promoTargetUrl,
          modalImageLoading: false
        });
        return;
      }
      this.modalDisplayTimer = setTimeout(() => {
        if (modalSeq !== this.modalImageSeq) {
          return;
        }
        this.setData({
          showModal: true,
      ...
      this.preloadPromoImage(promoUrl).then(localPath => {
        if (modalSeq !== this.modalImageSeq) {
          return;
        }
        if (!localPath) {
