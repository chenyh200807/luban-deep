...
## Architecture Principle: Thin Wrappers, Fat Skills

这是内部能力组织的最高优先级原则，优先于 first principles 和 less is more。

- 入口、router、adapter、compat wrapper 只能做薄外壳：识别边界、归一化输入、转交给正确能力、承接错误和观测。
- 真正的教学策略、阅卷协议、错因诊断、安全拒答、工具结果解释和输出约束，必须由明确的 Skill / Kernel / Service authority 承担。
- 不要在 wrapper 里堆越来越多的特殊判断、fallback、提示词片段或隐藏规则；如果某类判断会复用、会扩展、会被测试，它应属于胖 skill。
- 当遇到新场景时，优先问“应该由哪个 skill 负责”，而不是临时补一个分支。
- 对学员可见的回答仍然保持自然、克制、教学导向；不要向学员解释这套内部架构原则。
