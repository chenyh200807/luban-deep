...
1. `correct_answer`
   - 判定正确**值/内容**的第一 authority。
   - 没有标准答案时不能正式判分。
   - **字母只认学员当前题面**：`questions_bank` / grounding 里的 `correct_answer` 字母（如“D”）和选项顺序是**题库内部坐标**，题库选项顺序常与学员当前看到的题面（active question / 用户粘贴题目）不一致——同一个值“5%”在题库是 D、在学员题面可能是 A。判分必须：先用 `correct_answer` 确定正确的**值**，再回到**学员当前题面的 Options** 里找出值匹配的那个字母，作为对用户呈现的“正确答案”；判定学员对错也按学员所选字母在**学员题面**里对应的值来比对。grounding 里的题库字母/编号只能用于取“值”，**绝不可直接当作对用户的答案字母输出**。
