...
class SQLiteProductBehaviorStore:
    ...
    def _ensure_schema(self) -> None:
        ...
        with self._connect() as conn:
            ...
            # 手工索引清理(2026-07-26)：生产库里曾被直接执行过一条
            #   create unique index idx_pbe_playback_sequence on product_behavior_events(
            #     user_id, json_extract(properties_json,'$.playback_session_id'),
            #     cast(json_extract(properties_json,'$.sequence') as integer))
            #   where event_name = 'microlesson_playback'
            # 它在本仓零引用(不是这里建的),为一个当时未实现的功能预留,造成两个真实伤害:
            #   ① 表达式索引 + 部分索引让宿主 python 自带的旧 sqlite 直接
            #      "malformed database schema (idx_pbe_playback_sequence)" 打不开整个库,
            #      任何跑在宿主上的 BI/报表脚本静默读不到数据;
            #   ② 它把去重 authority 分成两处(主键 + 这条索引)。
            # microlesson_playback 的幂等由确定性 event_id 主键唯一保证
            # (见 luban_preview.record_luban_preview_playback),所以这条索引没有职责,
            # 直接删掉而不是补登记——少一个 schema 特性依赖、少一个决策点。
            conn.execute("drop index if exists idx_pbe_playback_sequence")
