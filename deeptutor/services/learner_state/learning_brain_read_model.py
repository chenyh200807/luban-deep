...
def _object_display(raw_id: Any, raw_type: Any = "") -> dict[str, str]:
    ...
    if object_type in {"training", "next_training"}:
        ...
        concept = _concept_label(parts[0]) if parts and parts[0].upper().startswith("1A") else ""
        error = _error_label(parts[1]) if len(parts) > 1 and parts[1].upper().startswith("E") else ""
    ...
    if object_type == "error" or str(object_id).upper().startswith("E") or ":E" in str(object_id).upper():
        parts = str(object_id or "").split(":")
        concept = _concept_label(parts[0]) if parts and parts[0].upper().startswith("1A") else ""
...
def _with_training_display(item: dict[str, Any]) -> dict[str, Any]:
    ...
    enriched.update({
        ...
        "display_meta": "；".join(item for item in (concept and f"知识点：{concept}", error and f"错因：{error}", focus) if item),
    })
    return enriched
