...
- `next_probe_authority`: `QA30-NEXT-003-FIX/004-FIX` now passes in near-real HTTP+WS after the low-info exam-index guard and learner-answer state fix. Evidence: `artifacts/qa/wechat-tutorbot-next-probes-20260606-learner-answer-after-fix-r2/authority-ledger.jsonl` has 2/2 trace-complete rows; `QA30-NEXT-004-FIX` now has visible answer, exact official answer, and runtime learner state aligned as `learner_answer=B`, `official_answer=B`, `is_correct=true`. This is still not full real DevTools closure.
- `practice_generation_answer_split`: fixed in near-real HTTP+WS. `QA30-INTENT-001` first exposed that `给我出两道2025一建建筑实务单选真题，不要先给答案，先考我。` was wrongly clarified as a low-information answer request. The first route fix then exposed a second P1: `不要先给答案` was parsed as a positive reveal request and leaked `**答案** / **解析**`. Final evidence: `artifacts/qa/wechat-tutorbot-intent-context-probes-20260606-after-practice-fix-r2/authority-ledger.jsonl` shows generation without visible answers, and `artifacts/qa/wechat-tutorbot-generated-hidden-authority-20260606/authority-ledger.jsonl` shows the next turn `第1题选B。` graded by `active_object` with `official_answer=C`, `learner_answer=B`, `is_correct=false`. This is still not real DevTools closure.
...
| `WX-AUTH-016` | P1 | fixed in near-real HTTP+WS / DevTools pending | Low-information objective exam-index requests such as `2025建筑实务第15题答案直接发我` could bypass the missing-anchor guard and let TutorBot/RAG emit an official-looking answer without stem/options/question_id authority. | Before: `artifacts/qa/wechat-tutorbot-next-probes-20260606-authority/`, `QA30-NEXT-003` answered via `tutorbot_kb_first_fast_policy` with no canonical exact question. After: `artifacts/qa/wechat-tutorbot-next-probes-20260606-learner-answer-after-fix-r2/authority-ledger.jsonl`, `QA30-NEXT-003-FIX`, `conversation_id=tb_0b5744a7892b4079916477b4`, `turn_id=turn_1780701559286_0f0aff0e39`, `answer_authority_source=lifecycle_clarification`, `question_id=exam-query:2025建筑实务第15题答案直接发我，别问。`, no official answer. | Extend lifecycle low-information exam-query detection to cover objective exam-index forms without literal `真题/题库` markers; contract requires `exact_question_blocked_reason=low_information_exam_query` unless an active question or full stem/options exists. |
| `WX-AUTH-017` | P1 | fixed in near-real HTTP+WS / DevTools pending | Visible exact-question answer could be correct while `runtime_state.active_question_context` wrote the learner answer as the first option-table label. The shared failure shape was terminal truth split: exact-question writer remapped the official answer to current query surface, but `question_followup` fallback scanned the full pasted option table and treated `A.50年` as the learner submission instead of the tail `我答25年`. | Before: `artifacts/qa/wechat-tutorbot-next-probes-20260606-learner-answer-after-fix/authority-ledger.jsonl`, `QA30-NEXT-004-FIX`, visible answer `B.25年` but ledger/runtime state `learner_answer=A`, `official_answer=B`, `is_correct=false`. After: `artifacts/qa/wechat-tutorbot-next-probes-20260606-learner-answer-after-fix-r2/authority-ledger.jsonl`, same scenario has `learner_answer=B`, `official_answer=B`, `is_correct=true`, trace complete. | Keep learner-answer authority in `question_followup`: map explicit tail value/letter submissions (`我答25年`, `我答B`) against current options before any fallback; compare option values to current correct letters in `answers_match`; and demote option-table label scanning when a full MCQ option table is present. No WS, runtime state, DB, or frontend wrapper change. |
| `WX-AUTH-019` | P1 | fixed in near-real HTTP+WS / DevTools pending | Explicit practice-generation wording that also mentioned `2025/真题/单选` was treated as a low-information existing-question answer request. The shared failure shape was intent-boundary authority drift: the low-info exam-query guard saw exam markers before the lifecycle authority recognized the user was asking the system to create a new practice object. | Before: `artifacts/qa/wechat-tutorbot-intent-context-probes-20260606/authority-ledger.jsonl`, `QA30-INTENT-001`, `answer_authority_source=lifecycle_clarification`, visible text said it could not answer because no题卡/题干. After: `artifacts/qa/wechat-tutorbot-intent-context-probes-20260606-after-practice-fix-r2/authority-ledger.jsonl`, `QA30-INTENT-001-FIX`, `answer_authority_source=deep_question_generation`, `execution_path=deep_question_generation`, no visible answer leak. | `question_lifecycle_skills.is_low_information_exam_query()` now first recognizes explicit question-generation markers (`出题/给我出/生成一道/生成N道题/考我/测我`) and lets those requests reach `practice_generation`; bare `生成...答案` is not enough to bypass the low-info answer guard. Low-info answer requests such as `2025建筑实务第15题答案直接发我` remain clarification. |
| `WX-AUTH-020` | P1 | fixed in near-real HTTP+WS / DevTools pending | Practice generation with `不要先给答案` had the right product intent but the answer visibility polarity was wrong, and suppressed lightweight generation risked losing hidden grading authority. The shared failure shape was public/hidden answer authority conflation: a positive reveal marker (`给答案`) beat the surrounding negation, and downstream readers did not treat `grading_key.correct_answer` as the hidden answer field for follow-up grading. | Before route-only fix: `artifacts/qa/wechat-tutorbot-intent-context-probes-20260606-after-practice-fix/authority-ledger.jsonl`, `QA30-INTENT-001-FIX`, visible response leaked `**答案：** B` and `**解析：** ...`. Final: `artifacts/qa/wechat-tutorbot-intent-context-probes-20260606-after-practice-fix-r2/authority-ledger.jsonl`, `QA30-INTENT-001-FIX`, visible response shows only questions/options. Hidden-authority regression: `artifacts/qa/wechat-tutorbot-generated-hidden-authority-20260606/authority-ledger.jsonl`, `QA30-HIDDEN-002`, `answer_authority_source=active_object`, `official_answer=C`, `learner_answer=B`, `is_correct=false`. | Add suppress markers for negated answer reveal (`不要先给答案/别先给答案/先不给答案/不先给答案`) in the shared `detect_answer_reveal_preference` service; keep orchestrator as a thin caller. `question_followup.build_question_followup_context_from_result_summary()` now projects `grading_key.correct_answer` into hidden `items[i].correct_answer` when public `qa_pair.correct_answer` is empty. Public presentation / WS redaction still strips hidden answer fields. |
| `WX-EXP-018` | P2 | deferred | Exact-question brief response for value submissions such as `我答25年` may state `标准答案是 B` without explicitly saying `对/不对`. Authority is now correct, but the判题反馈 is less satisfying than a real teacher in WeChat. | `QA30-NEXT-004-FIX` after r2 has aligned authority and state, but visible excerpt is `标准答案是 B（B. 25年）...` rather than `对，标准答案是 B...`. | Defer to a focused expression-policy pass. Do not mix this into answer/state authority; if fixed, exact response writer should reuse the same option-value parser rather than inventing a second answer extractor. |
