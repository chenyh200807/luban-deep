...
> 本目录收录鲁班智考所有**数据资产盘点**记录:原始资料、编译产物、考点统计、供给估算、母题适用性压测、计算公式、真题分值视觉核查等。
>
> **约定(每次数据盘点必做):**
...
| 2026-06-16 | [真题PDF分值视觉核查](2026-06-16-真题PDF分值视觉核查.md) | **视觉读真题 PDF 解析:每个采分点的分值齐全**(含计算步骤分/网络计划步骤分/0.5分粒度/部分给分规则);**订正前两版"per-point 分值缺/只7%"——分值在 PDF 解析里,只是没抽进 JSON;M4 是"视觉抽取"非缺料**(佑森解析估分,非官方) |
| 2026-06-16 | [深题供给估算](2026-06-16-深题供给估算.md) | 早期估算:6134 采分点仅 ~1-2% 可直接做深题;已被"313 深编译叶"结论取代 |

## 待办盘点(下次补)

- [ ] **视觉/OCR 抽取全 6 份真题 PDF 案例解析的 per-point 分值**(抽查 2021/2022 非佑森版式),回填 per_question `score:null`
- [ ] RichLeaf v3.2 token pack 与 313 深编译 runner 是否已合流(C 层公式与 v3.2 计算 leaf 未打通)
...
- [ ] 网络计划/流水公式的结构化(佑森讲义,未纳入计算公式盘点)
- [ ] 把计算公式汇总编译成结构化 formula registry
