...
> - 现有 Supabase RAG 知识库复用与来源映射，以 [ADR-006-supabase-knowledge-base-reuse.md](/Users/yehongchen/Documents/CYH_2/Markzuo/deeptutor/docs/openmaic/ADR-006-supabase-knowledge-base-reuse.md) 为准。
> - `建筑实务AI互动课堂_Implementation_Plan_v1.0.docx` 与 `建筑实务AI互动课堂_PRD_v1.0.docx` 仅保留历史上下文，不再作为实施 authority。

> ⚠️ 红线：
>
> - 本文件禁止直接派工。
> - 本文件中的 `web/components/exam-classroom`、`web/app/exam-classroom`、`exam_classroom capability`、`classroom_scenes`、`classroom_actions`、`exam_questions`、`/ask API`、scene 级发布状态等旧设计均不得实施。
> - P0 学员端主表面是 `wx_miniprogram`，宿主交付包是 `yousenwebview/packageDeeptutor`。
> - P0 必须先做 `Scene Runtime Core` + `WxRendererAdapter`，不能先做 Web-only player。
> - 当前唯一可派工计划是 [建筑实务AI互动课堂_Implementation_Plan_v1.2.md](/Users/yehongchen/Documents/CYH_2/Markzuo/deeptutor/docs/openmaic/建筑实务AI互动课堂_Implementation_Plan_v1.2.md)。
