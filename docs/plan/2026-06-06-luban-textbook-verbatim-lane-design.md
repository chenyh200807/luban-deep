...
## 9. Finer granularity + textbook ↔ question-bank coverage (2026-06-06)

**Finer node resolution (shipped).** A syllabus `node_code` is coarse (one leaf can hold 274 cards). The fine grain (197 `taxonomy_path` sub-topics) already lives in every signed record; it just wasn't indexed or used. Now: the manifest carries a `path_index` (taxonomy_path → point_ids, 197 entries, manifest-only so content_hash/signature unchanged); `resolve_node`/`build_pack_for_node` take `query`+`limit` and `_relevance_rank` (CJK bigrams + numbers + codes, deterministic, no embedding service) focuses a node's cards to the turn's top-K (default 12); `deep_question` passes the turn's stem as the query. Verified on real data: node `1A413030` (274 cards) → 12 focused on a 强夯 stem, rank-1 = the 强夯 夯锤 10~60t card. Whole feature flag-gated OFF; official scoring stays verbatim-only.

**Coverage report (`run_luban_textbook_question_coverage_report.py`).** Cross-references the signed pack against the 2026 question bank (`docs/2026/题库`, 1483 coded exercises). Taxonomy reality: textbook tags **20 leaf nodes + 197 taxonomy_paths**; questions tag **116 leaf nodes (`predicted_node`) and NO taxonomy_path**, at a different tree depth — so the exact join is the 6-char section prefix; per-path question counts are lexical-relevance ESTIMATES, labelled as such. Findings:
- **7 exam sections have questions but ZERO textbook cards** — the biggest gap. Largest: `1A4221*` 装饰装修与屋面工程相关规定 (36 questions, 0 cards), `1A4150*` 钢-混凝土组合结构 (18), `1A4140*` 水泥的性能和应用 (15). All 13 textbook sections do have questions (no textbook-card-but-no-question section).
- Block card production: 650/650 blocks have knowledge_cards; only **2 blocks signed 0** (the mis-attached synthesis cards from §8).
- 75 taxonomy_paths have 0 relevance-estimated questions (heuristic; textbook content possibly under-tested).

**Root cause of the gaps (the real finding):** there is no single canonical taxonomy both corpora are pinned to. Textbook and question bank were tagged independently against the same syllabus tree at different depths, with inconsistent free-text `taxonomy_path` strings (e.g. `项目管理实务 > 安全管理 > 施工安全管理` vs `项目管理实务 > 施工安全管理`). Coverage can only be exact once both sides pin to one canonical node spine (the official 2026 考试大纲 tree, which the `node_code` + `parent_code` already approximate).
