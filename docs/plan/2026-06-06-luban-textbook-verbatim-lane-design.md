...
**Conclusion:** `content_markdown`'s vision-OCR is high-fidelity; the garbled PDF text layer does not affect us; no corpus re-OCR is warranted for the signed pack. Audit ledger: `artifacts/luban_grading_artifacts/textbook_knowledge_full_20260606/vision_ocr_drift_audit.json`. Known nuance for a future pass: calc/case cards may carry enrichment-derived (computed) values in key_numbers; a `printed_vs_derived` flag could separate them.
