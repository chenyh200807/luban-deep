...
- 文档路径：`/docs/plan/2026-04-15-bot-learner-overlay-service-design.md`
- 创建日期：2026-04-15
- 适用范围：完整 TutorBot、多 Bot、Learner State、Heartbeat、Guided Learning、Notebook、Promotion Pipeline
- 状态：Implemented foundation v1（服务基础已实现；多 Bot 产品闭环仍看主 PRD gate）
- 关联文档：
...
本设计稿的 repo-side 实施入口已经固定：

- Overlay service：`deeptutor/services/learner_state/overlay_service.py`
- Overlay migration：`supabase/migrations/20260415000200_bot_learner_overlay.sql`
- RLS migration：`supabase/migrations/20260419000100_learner_state_rls.sql`
- Heartbeat arbitration：`deeptutor/services/learner_state/heartbeat/arbitration.py`
- Member Console 治理面：`deeptutor/services/member_console/service.py`
