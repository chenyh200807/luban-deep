...
- [2026-06-04-luban-grading-engine-master-control-plan.md](../总控入口与当前作战图/2026-06-04-luban-grading-engine-master-control-plan.md) §0.12.6（"Nexus-style compiler 是能力放大器，不是题库闸门"）、§0.26（authority-aware open-world expert system）、§0.26.14（知识编译 storage/serving contract）、§0.26.9 M27（五面同消费 `luban_context_pack.v1`）。

并行轨（**不同主线，不要混**）：

- [2026-06-08-luban-production-canonical-activation-authorization-package.md](../总控入口与当前作战图/2026-06-08-luban-production-canonical-activation-authorization-package.md)（M33-ACT 生产/canonical 授权门）—— 那是**评分 release-truth 上生产**；本计划是**教学知识泛化**，不需要任何授权门。
- [2026-06-08-luban-grounded-remaining-audit.md](../总控入口与当前作战图/2026-06-08-luban-grounded-remaining-audit.md)（接地气剩余审计）。
