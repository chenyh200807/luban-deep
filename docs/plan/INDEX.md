...
| [2026-05-24-luban-assessment-testset-p0a-execution-plan.md](2026-05-24-luban-assessment-testset-p0a-execution-plan.md) | Execution Plan | Proposed v1 (2026-05-24) | 将 Assessment TestSet PRD v0.3 拆成 P0A 可执行 Phase -1/0/1：先做 reality check、topic granularity、payload redaction baseline、durable `assessment_sessions` schema/RLS/idempotency 设计和 `last_assessment` audit；再做 coverage dry-run 与教研 signoff；最后实现 durable session repository、scoring、per-item `learning_evidence`、attempt refs、mistake-book writeback、result report 和佑森 run/result view-model gates。 |
