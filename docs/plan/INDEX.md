...
| [2026-05-26-luban-learner-workspace-notebook-calendar-prd.md](2026-05-26-luban-learner-workspace-notebook-calendar-prd.md) | PRD | Proposed v0.3 | 基于 LLM Wiki / Obsidian Wiki 调研与代码实证复审，把学情页产品表面升级为“AI 实务教练 + 个人笔记本 + 学习计划日历 + 学情看板”。v0.3 将 P0A 前置 blocker 收紧为：notebook 手动收藏必须切轻路径，不能触发 summary rewrite / compiled-truth refresh；首页只扩展 `/api/v1/mobile/learning-report`，不新增 `learner-workspace/home`；P0A 不做 planner CRUD；生产 P0A 必须解决 notebook durable store / RLS / 并发写入或标为单端内测。 |
...
| [2026-05-26-luban-learner-workspace-notebook-calendar-p0a-execution-plan.md](2026-05-26-luban-learner-workspace-notebook-calendar-p0a-execution-plan.md) | Execution Plan | Proposed v0.1 | 把学情工作台 PRD v0.3 的 P0A 落成可执行 TDD 任务。核心架构判断：新增 owner-scoped `learner_notebook_cards` Supabase 表（仿 `learner_mistake_book_items`，per-row + version 乐观并发 + RLS）+ 新 fat-skill 权威 `NotebookCardService`，让 workspace 卡片**不经过** `NotebookManager._writeback_learner_state()` 重路径——一次性收权 `refresh_from_turn`（summary LLM 改写）、`patch_overlay`（Bot-Learner Overlay 污染，PRD v0.3 漏列、本计划补入禁止模式），卡片只发一条 `source_label=student_note` 低权重 recall 事件；legacy solve/guided_learning notebook 完全不动（零回归）。Phase 0 现实校验/污染基线 → P1 durable 表+RLS 迁移 → P2 `NotebookCardService` 轻写回+recall 降权 → P3 `POST /api/v1/notebook/add_record` 按 `metadata.card_type` 分流+卡片 PATCH/DELETE 409 乐观并发 → P4 只扩展 `build_learning_report_read_model`（`note_assets`+只读 `today_tasks`，无第二首页 reader、无 `my_created` 来源）→ P5 probe `measurement_confidence` 门槛 → P6 wx/yousen 双端 UI + 微信开发者工具回归。用户已拍板 durable store 先行（非单端内测）。迁移不在代码 PR 内 apply（独立 release gate）。 |
...
| [2026-05-26-luban-learner-workspace-notebook-calendar-prd.md](2026-05-26-luban-learner-workspace-notebook-calendar-prd.md) | PRD | Proposed v0.3 | 学员端“我的学情工作台”产品 PRD：把 AI 自动学习档案、用户手动笔记、个人采分点手册、错因地图和计划日历合成一条“沉淀 -> 行动 -> 复测 -> 改善”的交互闭环；v0.3 修正手动笔记污染 summary/recall、第二个首页 read model、P0A planner store 矛盾、file-backed notebook 持久化四个 blocker。 |
