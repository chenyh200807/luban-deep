...
| [2026-05-21-luban-learning-report-world-class-optimization-plan.md](2026-05-21-luban-learning-report-world-class-optimization-plan.md) | Implementation Plan | Proposed P0-P2 / Expert Re-evaluated | 针对系统审查报告形成的顶级学情页优化实施计划：把学情页升级为“学习复盘 + 作答证据 + 云端错题集 + 下一步训练 intent + 对话首页个性化”的学习操作系统；2026-05-21 专家复评后将 `Learning Evidence Quality Gate` 提升为 Task 0，并明确系统答疑解析复用 `learner_memory_events.learning_evidence[event_type=conversation_learning_evidence]`，对话首页个性化复用现有 `home_dashboard.today_focus/recommended_prompts`，不新增第二套 learner memory 或首页推荐 authority；P0 先交付 detail-ready evidence、attempt detail、云端错题集、training intent、系统答疑解析沉淀、首页今日焦点/推荐问题联动、稳定结论分区这条闭环，再做 shared view model、UI 重构、`MasteryEstimator`、E2E 和 10 万学员性能门槛。 |
