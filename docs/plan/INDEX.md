...
| Harness Engineering 收敛与世界级化 | [2026-05-27-luban-harness-engineering-single-authority-world-class-execution-plan.md](2026-05-27-luban-harness-engineering-single-authority-world-class-execution-plan.md) | 横切收敛主线（v2 经 codex 对抗审查后收缩重写）：chat 与 tutorbot 两套执行壳保留（SLA 不同），但 scene/grounding/exact 三类业务事实强制收敛到各自唯一 authority——scene 唯一权威是 `question_lifecycle_skills`（非 teaching_modes，后者是 legacy shim 待降级），两壳只能读 turn metadata、禁止重判。当前**只允许执行窄 P0**：trace 级 golden 基线 + scene 权威强制 + grounding/exact 按题型分叉行为矩阵（不做"只搬不改语义/字节级一致"承诺）+ `check_harness_authority.py` 护栏。统一 kernel（已撤销）、chat 迭代化、可恢复压缩、KV-cache 稳定前缀、model-swap 抽象全部列入 **Deferred Roadmap**，gating：P0 完成 + launch-readiness/assessment 主线稳定后再排。`Proposed`。 |
