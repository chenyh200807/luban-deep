...
| [2026-05-18-deeptutor-learning-fact-retrieval-implementation-plan.md](2026-05-18-deeptutor-learning-fact-retrieval-implementation-plan.md) | Implementation Plan | Partial local implementation / Live gate pending | 学习事实召回增强子计划：在现有 `RAGService` / `SupabasePipeline` 内实现显式 query plan、compiled truth source group、provenance-aware ranking、typed graph expansion、maintenance skill 与 eval gate；专家复审后要求先走 trace-only / shadow-source / weak-point-only 的分阶段上线，不新增第二套 RAG 或聊天入口。 |
| [2026-05-18-deeptutor-learning-fact-retrieval-gap-closure-plan.md](2026-05-18-deeptutor-learning-fact-retrieval-gap-closure-plan.md) | Gap Closure Plan | Active | 针对 learning-fact retrieval 子计划剩余差距做收口：live gate、Phase D 弱点召回生产验收、graph expansion 接入 RAG、TutorBot runtime compiled truth 传播、maintenance dry-run workflow、contract guard 与计划状态。 |
...
| [2026-05-18-deeptutor-learning-fact-retrieval-implementation-plan.md](2026-05-18-deeptutor-learning-fact-retrieval-implementation-plan.md) | Implementation Plan | Partial local implementation / Live gate pending | 把 GBrain 的 query understanding、hybrid/RRF、compiled truth boost、provenance、typed graph、dream cycle 和 maintenance skills 映射成 DeepTutor 学习事实召回计划，明确 authority 顺序：exact 题库事实 > 标准/教材证据 > compiled learning truth > 普通语义 chunk；首个可交付切片限定为 query plan trace、compiled truth shadow、provenance trace 和 weak-point-only enablement。 |
| [2026-05-18-deeptutor-learning-fact-retrieval-gap-closure-plan.md](2026-05-18-deeptutor-learning-fact-retrieval-gap-closure-plan.md) | Gap Closure Plan | Active | 学习事实召回剩余差距收口计划，覆盖 graph-aware compiled truth docs、TutorBot RAG propagation、maintenance dry-run、contract guard、live direct RAG / public WS / Langfuse evidence。 |
