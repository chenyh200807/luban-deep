...
3. `scripts/run_readiness_check.py --check-id wechat_devtools` -> `wechat_devtools`，默认执行 `scripts/run_wechat_devtools_daily_smoke.py`，DevTools project root 是 `yousenwebview`，目标业务面是 `packageDeeptutor` 分包。
