...
| **3** | orchestrator.py:332-341(case)+376-380(mcq) 接 question_review 已有的 `_resolve_turn_semantic_decision` LLM 门（237-253/306-322 镜像），仅 LOW/None scene 调 LLM，HIGH 仍 bypass | 中 | 低置信交 LLM 判非作答→不判分；高置信仍直达判分 |
| **4** | semantic_router.py:760-766 守卫改"HIGH 不翻案，LOW 允许 history-aware LLM 复核"（真正打破 shielded-from-veto，放最后因改最敏感不变量） | 中 | LOW submission 经 LLM 翻案为 followup；HIGH 不被翻案 |
| **5** | 删 deep_question.py:3847-3881 legacy 二次 resolve（与 3753-3765 重复）+ 3656-3744/fabrication 点 demote 只读 canonical（**先证 router 覆盖真作答再删兜底**） | 低（次序严格） | 全量 deep_question 判分测试 + 6 失败签名 + 真作答必判三连跑 |
