...
- 文档路径：`/docs/plan/2026-04-15-bot-learner-overlay-prd.md`
- 创建日期：2026-04-15
- 适用范围：完整 TutorBot、多 Bot 场景、个性化、Heartbeat、Guided Learning、跨功能 learner state
- 状态：Partially Implemented v1（repo foundation；多 Bot 产品闭环未全关）
- 关联文档：
...
本 PRD 已有 overlay service、migration、测试和运营面入口，但不能算多 Bot overlay 产品完成：

1. `BotLearnerOverlayService` 只承载局部 overlay 字段，禁止承载 profile/summary/progress/goals 等全局 learner truth。
2. promotion candidate 通过统一 `apply_promotions` 写回 `LearnerStateService`，不允许 overlay 直接成为第二份全局事实。
3. 多 Bot heartbeat 由 `LearnerHeartbeatArbitrator` 做全局仲裁，overlay 只能提供 priority/suppress 等候选信号。
4. Member Console 已提供 overlay 查看、patch、promotion apply/ack/drop、events/audit 查询。
5. 未完成的产品 gate：真实多 Bot 线上流量策略、promotion 质量抽检、运营 SOP 权限配置、仲裁策略长期效果验收。
