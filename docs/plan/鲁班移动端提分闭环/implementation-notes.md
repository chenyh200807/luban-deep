...
### 2026-07-11（基建三件之① 尺接闸 · 宏观审计工单落地 · 即时入账）
- **[风格泄露闸接电]** `promote_variant_bank.py` 变体 kind 默认闸追加 `_run_style_tells_gate`（调 audit_variant_style_tells --gate --pack）——审计尺从"挂在墙上"变成人闸硬检查。**棘轮语义**：存量 11/17 带病池下次重签会被拦（有意压力：不完成模板对偶返工不得再过闸）。双向行为验证：X02(干净)放行 / X01(LEAK)被拦。
...
### 2026-07-11（两条前端线合流：老蓝首跑线 × 轻练/复习模块线 · owner拍板即时入账）
- **[owner三裁决]** ①新版五模块前端底座废弃, 打 tag `deprecated/new-fivemodule-frontend-20260711`(内容资产不废,只废壳);②两线早合流;③以后前端工作一律在老蓝首跑线。
- **[合流原则]** 分支 `integration/old-blue-plus-luban-modules`(基=老蓝快照 a6002360, 合入=seethrough线 a6444610, 47冲突)。壳层(tab bar 4tab/chat/report/profile/login/billing/register/freeCourse/assessment/history/app.js)=老蓝权威逐字节保留;模块层(retest纸墨版/seethrough 5关连闯/review/errorbank/gauntlet/concept-cards 141卡/paper-ink)=看穿线权威;后端=双方并存(老蓝 MCQ light-practice wip + 我方 retest pool/编译资产链 + main 的 member/eval治理);turn_runtime 双修法合成(老蓝87ad68c9助记流×main e1a3de48练题上下文)。
- **[入口接线]** learn页(五模块壳)不移植;seethrough/review 在老蓝壳的入口=handoff交接页新增两钮(趁热5关连闯/看我的复习安排)+错因银行原有钮;route.js 补 seethrough/gauntlet 白名单与 helper;app.json 注册 review/concept-cards/errorbank/gauntlet/seethrough。
- **[验证]** 后端定向 424+218 passed(read_model活体计数更新为141卡重签后8/1/3/5;member fixture union修复);js套件失败22→3, 剩3个(chat_points_sync/report_view_model/freeCourseDetails_polyv)为老蓝线自带红(逐字节同老蓝仍FAIL,含owner拍板本地签名vs旧安全测试矛盾),非合流引入。
- **[Deviations 待补]** ①首体验漏斗前端埋点(module_viewed/auth_authorize_clicked等5事件)未随壳移植,老蓝login/chat无嵌点,需单独按老蓝壳重嵌;②surface-telemetry pre-auth pending queue 未移植;③渠道归因(reg_channel)前端透传未移植(后端已并入,参数可选不破);④双轮spike登录落地翻页(flags.shouldLandOnDoubleWheel)随learn页废弃。
