...
## 2026-07-27 · 语料瘦身:2.5MB 里装的是"零"和"残留";以及一次 worktree 丢工事故

**①现象**:审计瘦身与拆台账做完后,生产语料仍有 5.36 MB。owner 要求继续做
"全零骨架惰性物化"与"墓碑压缩"。

**②发现路径(先量化再动手 —— 上一刀刚因收益不足被回退,这次先测)**:
在容器内按真实语料**只读计算**(不写任何远端文件),模拟两刀后的体积:
- `chapter_practice_stats`:1308 名会员 **distinct=1、合计 1.1 MB** —— 1308 份字节
  完全相同的全零结构;`chapter_mastery` 同样 distinct=1、943 KB
- 墓碑 613 个(46.9%)吃掉 members 字节的 **47.5%**
- 模拟结果:5,361,236 → 4,259,900(刀一 −20.5%)→ 3,367,403(累计 **−37.2%**)
**先拿到这个数才动手**,与上一刀(deepcopy)的教训直接相关:那刀实测只省 8% 被回退。

**③分析**:病是"为『零』预留物理空间"。"该章节没练过"与"该章节键不存在"语义完全
相同,却给每个会员物化一份 844 B 的零;墓碑同理 —— 学习数据在合并时已并入 canonical,
留在墓碑上的是纯残留副本。这不是性能问题,是**把缺省提前物化**的建模问题。

**④修法**:写入侧停止预填 + 两个幂等 migration 回收存量。安全性靠**逐点核实消费者**
而非猜测:真实写入点都是 `setdefault(章节,{...})` 后累加(三处)、读取点都 `.get(...) or {}`
容错;墓碑的 `merged_into` 消费点全仓只有"沿链跳 canonical / 统计 canonical_root /
overlay 链",没有任何路径读墓碑的学习数据。回收只删全零条目,**有真实练习数据的一律
保留并有反例测试守着**。

**⑤本轮事故:worktree 被并行工作线清理,未提交改动全部丢失。**
我在 `deeptutor-billing-catalog` worktree 里写完两刀(未提交),下一条命令的 `cd` 报
"no such file or directory" —— 另一条工作线(`chore-cleanup-worktrees-and`)清理了它。
`cd && python3` 因短路未执行,而**同一条 Bash 里换行分隔的 pytest 却在主工作区跑了**
(14 passed 是主工作区结果,当时被我误读成"改动生效了")。
第一反应是核实主工作区有没有被污染:`git status` 该文件无改动 + 我写的四个符号命中数
全 0 ⇒ 未污染,owner 的未提交工作完好。改动本身重做了一遍(设计完整,很快)。
**教训**:①**长任务中的改动要尽早 commit**,worktree 不是安全的暂存区 —— 在有并行
agent 清理的仓里尤其如此;②`cd X && cmd` 与下一行命令是两条独立语句,`cd` 失败只短路
本行,后续行仍在**默认 cwd** 执行 —— 跨目录批量操作应用**绝对路径**而不是靠 `cd`;
③读工具输出要逐行对齐预期,"有 pytest 通过输出"不等于"我的脚本执行了"。

**⑥验证**:308 passed(member_console 全量)+ 359 passed(mobile/BI/学情)+ 4 条守卫
(含"有真实练习数据必须保留"与"两个 migration 必须幂等" —— 后者若破,每次读都会重写
整个语料);contract_guard + ruff 绿。契约新增两条存储纪律。

---

## 2026-07-26/27 · 审计无界增长:我要修的那个方案本身会造成资损;我为修它加的东西又拖垮了 CI

**①现象**:生产 `member_console.json` 8.96 MB 且加速增长(审计字节量周环比 +19%/+53%,
7–9 周破 20 MB)。owner:"无界增长不能接受,要根治"。

**②三次打脸(每次都比结论值钱)**:

- **打脸 1:我给 owner 的建议本身会造成资损。** 我上一轮说"下一个 PR 加轮转"。专家挖出
  `audit_log` 不只是审计,它同时是**冲正的金额权威**(`_find_latest_manual_membership_purchase_audit`
  / `_has_manual_membership_reversal` 线性扫它;原注释自承 *the only amount authority*)。
  按时间 TTL 只删掉 purchase/reversal 配对中一条 ⇒ **同一笔被冲正两次** 或 旧单静默不可冲正,
  **且是保留期到点才爆的延迟引信**。⇒ 教训:**给一个数组加保留策略前,必须先证明它只承担
  "审计"这一种职责。**
- **打脸 2:我的根因命名只对三分之一。** 我说"审计与业务状态同居一个存储单元",专家给出三层:
  L1 **存储的读/写/加锁最小粒度=整个语料**(双向证伪:删光 audit 病不消失、换行式存储病消失
  ⇒ 病在**粒度**不在**共处**);L2 无界 append-only 同居;L3 **一个数组承担审计/交易台账/
  产品埋点三种保留期互斥的职责**。
- **打脸 3:专家情报在我动手时已过期。** 专家测出"42% 体积是 indent=2 空格,一行省 3.79MB",
  我去改时发现**另一条工作线(#569)已做完只是没部署**。本轮 main 在我工作期间前进了 6 次。
  ⇒ 印证专家自己写的告诫:读代码得出的结论,动手前必须对 `origin/main` 复核一次。

**③行业对照的反直觉结论**:量级仅 **10³–10⁴ 条/年,比选型决策表最小档还小三个数量级** ⇒
"问题 100% 是编码方式+存放位置,0% 是数据量",任何 ClickHouse/Kafka/CDC 提案当场否决。
且**本仓已把这件事做对过三次**(admin_store jsonl sidecar / usage_ledger /
product_behavior_store),`member_console.json` 是漏网实例 —— 不是架构设计题,是补齐已有模式题。

**④修法(两刀,顺序不可颠倒)**:
先**审计瘦身**(#572):只存变化字段,支付结算三类 action 例外保完整 after(冲正读它)。
实测 5615B→162B(省 98%),用户自助占审计字节 93.6% ⇒ 增长率降约 88%。刻意不碰引信。
再**拆冲正台账**(#575):结算事实进显式 `membership_purchases`,`reversed_by` 非空即不可再冲
(O(1) 唯一约束),三个判定改台账优先、audit 仅作存量回落。**返回形状保持兼容,冲正逻辑一行未改**。

**⑤我自己造的坑(本条最该记住)**:#572 首版按专家建议旁存 `before_digest`/`after_digest`
化解"diff 算错不可检测"。**它对 before/after 各做一次全量 `json.dumps` —— 在一个"存在理由
就是消除全量序列化"的改动里加了两次全量序列化。** CI `api-contract` shard 从 3m19s 涨到
**12m15s 撞 `timeout-minutes: 12` 被取消**;本地 927 passed 5m43s 完全跑不出来(本地没有
12 分钟墙)。移除后 CI **2m57s**(比基线还快)。
⇒ 教训:**给性能改动加"锦上添花的正确性保障"时,先算它的成本量级 —— 与被优化的开销同量级
就不是锦上添花,是把改动抵消掉。** 另:本地全绿 ≠ CI 全绿,当 CI 有 job 级 timeout 而本地没有时,
慢 4 倍不会在本地暴露。已用 `test_audit_entry_adds_no_full_serialization_cost` 守住。

**⑥拆台账时又只拆了一半,靠反例测试逮住**:改完 `_has_..._reversal` 查台账后,我跑"清空
audit_log 再冲一次"验证 —— 被拒绝了,但**理由是 `not found` 而非 `already reversed`**。
说明第三个函数 `_find_latest_..._audit` 仍在扫 audit ⇒ 只消除了"二次退款",没消除
"正常冲正 fail-closed"。补齐后两个场景才都对。
⇒ 教训:**验证要看"拒绝理由"而不只看"是否被拒绝"** —— 碰巧安全和结构安全长得一样。

**⑦验证与部署**:304 passed + 265 passed + 4 条台账守卫(含两条故意清空 audit_log 的反例);
部署 `ba36e95a1` 后生产文件 **8,957,000 → 5,343,682 B(−40.3%)** 且已紧凑,
容器内单条审计实测 **3657B → 59B(省 99%)**。
**残留**:`_load_member_snapshot` 整库 deepcopy(比 json.loads 贵 3 倍、几乎每请求触发,
实测全站吞吐上限约读 11 次/秒)、members 全零骨架(35%)、合号墓碑(46.8%)、分级 retention
(交易 3 年/鉴权 6 月/埋点 90 天,依据网安法+电商法+PIPL)——**retention 现在才被解锁**。
...
零实现,要求**去掉班主任督学**并**表明"9.9 元 30 个教学视频 / 68 元全部教学视频"**。

**②发现路径**:字面执行只需改 5 个字符串。但一查就发现两个拦路事实:
- **`desc` 不在我上一轮设的 pinning 白名单里** ⇒ 生产持久化目录里的旧 desc 不会被覆盖,
  **改了种子对存量不生效**。这是上一轮把 desc 归类为"营销文案、运营可改"的直接后果。
- **视频权益的真值是 `mobile.py` 里一个手写 frozenset**(`{light_98,vip,svip,supreme_svip}`
  + 一条 starter_19 特判),与 desc 文案**两处独立维护**。只改文案 = 承诺与发放各写一份,
  下次新增档必漂移 —— **正是 owner 这次要修的那个病的新实例**。

**③分析**:一等业务事实是「一档套餐给多少个教学视频」,它此前没有唯一权威:文案在目录、
发放在 router 枚举、契约散文里还抄了第三份。`desc` 的分类也判错了 —— 它不是营销包装,
是**对外权益承诺清单**(付费墙直接渲染),属产品定义;运营不该能把它改成承诺一项系统
并不提供的服务(那正是消费者权益纠纷的成因)。shared shape = `authority 碎片化`。

**④修法与理由**:`teaching_video_limit` 成为档位目录的**字段**(None=全部,正整数=上限),
`resolve_teaching_video_limit` 直接读它,**删掉 frozenset 与 starter 特判**;
`desc` 与 `teaching_video_limit` 一并进 `_PINNED_SEED_FIELDS`(承诺是产品定义),
`label/badge/audience/original_price` 仍留给运营。契约散文改成**指向目录**而不再自抄一份
tier 清单。新增测试直接断言「desc 承诺的数字 == 实际发放的上限」,让文案漂移不可能静默存活。

**⑤顺带修掉一个已知生产坑(本轮最意外的收益)**:归一化此前**不认识 `teaching_video_limit`
就丢弃它**,而磁盘上有这个历史字段 ⇒ `normalized != data["packages"]` 恒真 ⇒
**每个读请求都在写盘并持跨进程 `LOCK_EX`**,还不断改 mtime 让任何 mtime 缓存永不命中
(这条正是同月 BI 性能战役留下"只留证据不动代码"的那个坑)。归一化保留该字段后,
实测三次 `_load()` 的 mtime 不再变化。**教训:一个字段"不被认识"不只是丢数据,
它会把整个存储层的幂等性打破。**

**⑥验证与教训**:权益 11 组行为矩阵逐条不变(20/30/None),另新增别名用例
(`VIP`/`light_99`/`STARTER_19` 现在也正确归一 —— 收权时我一度漏掉别名归一化,
自测 13 组时才发现并补回 `_canonical_package_id`);265 + 全目录回归、19 道闸全绿。
**教训**:①owner 的字面要求("改文案")有时**必须**扩大到收权,否则等于新增一处名实不符,
但扩大范围要说清为什么;②**YAML 多行块里绝不能出现 `": "`** —— 这一轮我又踩了两次
(`teaching_video_limit`: null / Current catalog: 20),上一轮已踩过同样的坑。

---

## 2026-07-25 · 计费档位:病不是"定价有两个权威"而是"档位这个对象从未存在" → 我的修复引入了 BLOCKER,被对抗 review 逮住

**①现象**:owner 要求综合审查已上线的 `main`。20 道治理闸全绿、155 个端点鉴权无越权、
微信支付回调无可利用漏洞 —— 但四轴静态审查挖出一条完整资损链。

**②发现路径(值钱的是被推翻的那几条)**:

- **起点假设(部分对)**:我命名为"套餐定价被复制成多份按档位手写的枚举"。四个家族专家
  并行后规模从 5 处扩到 **30 处**,并各自挖出我没看到的东西:`days` 目录声明 180 而支付
  链路硬编码 365(专家 A 与专家 D **独立同源确认**);`_MEMBERSHIP_TIER_RANK` 漏两个付费档;
  `desc` 卖的 7 项能力有 6 项无 tier enforcement、"班主任督学"全仓零实现。
- **歧路 1:我把 `days` 定性成"2 倍权益超发违约",方向反了。** 指挥官用三条硬证据证伪:
  `billing.wxml` 全文 137 行只渲染 5 个字段,**唯一的"量"承诺是 `{{item.turns}} 次`不是天数**;
  全仓 grep 前端零命中任何有效期承诺;git 时间线显示 `days:180` 被夹带在一个"排除 eval
  账号"的提交里、**同一提交没有一行把它接进支付路径**,从落地起从未结算过任何一笔。
  ⇒ 不存在被违约的用户。正确修法不是"把 365 改成 180"(那是无依据地砍新购用户权益),
  而是**把死字段接活 + 常量对齐既成事实**,行为零变更,从此"要不要 180"变成改一个数字的产品决策。
- **歧路 2(最严重):我的第一版修复引入了 BLOCKER,而且我的注释在为它造假。** 我把兑付路径
  对 archived 档的硬拒改成告警(修资金滞留是对的),注释写"下架不可售的正确防线在收钱前的
  checkout" —— **那道防线不存在**:checkout 读的是不带 `status` 键的原始种子,结构上看不到
  下架。净结果是把 status 从"一道坏闸"变成**零道闸**,运营 archive 998 想阻止它被卖,改动后
  用户会顺利拿到 5 万点。**这正是 root-cause skill 警告的"绕开 authority 而非恢复 authority"。**
- **歧路 3:我的第二版修复把展示/售卖收敛到持久化目录,又踩了性能坑。** 同一天 main 的 BI
  方法志里记着一条:`packages_changed` 恒真 ⇒ 每次 `_load()` 全量写盘 + 持跨进程 `LOCK_EX`。
  我等于把高频 `/billing/wallet` 接到了这条路径上 —— **刚删掉一处重复同步读,又加回一处**。
  实测 `_normalize_package_catalog(None)` 零 IO 且快 11 倍,同样给出 status 与 998 的 archived,
  改用它:20 次调用 0.202ms、mtime 不变。
- **歧路 4:我犯了自己反复警告的假绿。** 首次跑 contract_guard 用了 `cmd | tail`,`$?` 拿的是
  `tail` 的退出码,报了"exit=0 通过"。真实退出码**一直是 1**。
- **歧路 5:测试的"反同源"我只做了一半。** 我把 parametrize 派生自 `_default_packages()`,
  review 指出**期望值也从种子派生 = 自证**:有人把种子 days 改成 1,21 个 case 全绿而每笔
  会员时长崩塌。正确切分是**覆盖动态派生、真值静态钉死**(金标表)。
- **歧路 6:基线红被我误当成自己改红。** 全目录跑出 5 failed,其中 4 个在
  `test_home_dashboard_learning_projection.py`。去只读基线 worktree 跑同一文件,**逐字相同的
  失败**⇒ pre-existing,与本次无关。单独跑 ≠ 污染,必须回基线对照才能定性。

**③分析**:真病因不是"定价有两个权威"。**是"档位"这个一等业务对象从未被建模成一份可执行
的数据,只被建模成一串散落各处的手写字面量。** 一档套餐是什么(价格/点数/时长/权益上限/
能否售卖/是否上架)本应是一条记录,系统里却没有任何一处持有它 ⇒ 每个消费点被迫重新发明
它的一个投影 ⇒ 每个投影都是手写枚举、都可能漏档 ⇒ **而测试也读同一份枚举,所以缺陷结构上
不可证伪**。"被复制成多份"是症状,"那个对象不存在"才是原因。
shared failure shape = `producer/consumer granularity mismatch` 的变体 + `authority 碎片化`。
另有两个**正交**病(收权治不了,必须分开):产品实现缺口(desc 承诺无 enforcement)、
存储/async 契约(82/340 端点在事件循环上做同步 IO、`audit_log` 7.3KB/条无界)。

**④修法与理由**:展示/售卖/兑付收敛到同一份归一化 canonical 目录,`status` 第一次成为可执行
的可售性权威;998 在**种子里**声明 `archived`,产品边界从前端搬到服务端。pinning 从三段
`if/elif` 改为按 id 整档覆盖但**只锚定白名单**(经济向量+可售性+与 turns 耦合的 per)——
review 实测证明把文案一起钉会静默吞掉 BI 保存(提示成功、下次读回旧值、无 audit),那是把
资损防线扩成越权。档位排序改**按价格**派生而非目录列表下标:目录顺序同时是付费墙卡片展示
顺序,营销重排不该静默改变合号优先级。兑付侧显式 `sellable_only=False`,因为钱已到账后
解析失败=资金滞留。

**⑤验证与教训**:member_console 309 passed(4 个 pre-existing 已回基线对照)、支付/钱包/BI
311 passed、20 道闸 + contract_guard 全绿;**真实数据面实测**可售目录=4 档、998 不可自助
购买、兑付侧仍可解析、零文件触碰。三个教训:**(a)** 注释里写"防线在别处"之前必须去证明那道
防线真的存在 —— 否则就是给自己的绕行造假;**(b)** 反同源枚举要分两个维度:覆盖可以派生,
真值必须静态;**(c)** `cmd | tail` 会吃掉退出码,这是我自己记过的坑又踩一次。
**未做(诚实边界)**:运营自建档仍不可自助购买(与改动前一致,非回归);前端两份档位副本、
`audit_log` 轮转、82 处 async 阻塞、desc 承诺无 enforcement、合号不搬钱包余额、
"点数不过期 vs 视频权益按期消失"全部留独立 PR,已在 PR 描述里逐条列明。
