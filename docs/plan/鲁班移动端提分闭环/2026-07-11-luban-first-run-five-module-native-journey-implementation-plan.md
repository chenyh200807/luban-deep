...
> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. 本任务未经 owner 授权使用 subagent 或提交 commit，因此本轮采用当前会话 inline execution，并以逐任务 diff/test checkpoint 代替 commit。
...
当前终态：`implemented_and_local_authenticated_pass_but_release_blocked`。`real_wechat_package` 页面级旅程与本地 auth-chain 均 PASS；唯一剩余 release stop condition 是四题双教研 signed verdict。当前不 commit、不 push、不部署。

本轮 fresh verification：后端聚焦集 `196 passed`；前端首次旅程、学习入口、注册、五 Tab shell、首页 prompt、旧摸底 authority、learn view model、seethrough page 共 9 个 Node 脚本 exit 0；contract、schema、REST route、双 index mirror、whitespace gate 全部通过。DevTools 真页面验证：答题态五 Tab 隐藏，`稍后 -> 回学习` 后恢复五 Tab；`custom-tab-bar` 三个实现文件相对正确产品视觉基线零 diff。
...
- Git：未经 owner 明确授权不 commit、不 push、不 merge；只保留窄 diff 和测试证据。
