...
> **⚠️ gold 有效性核实(2026-06-14,做 Stage 4 前必读)**:`luban_case_grading_golden_v1.json` **其 redline 自承"两者皆 AI,非 AI-vs-人类真相;PO 抽查为唯一人类锚;顶级 IRR=v1 待真人",status=`PO_signoff_pending`**——拿 AI coverage 判分对它=**循环假绿**,不能当 Stage 4 生产门。真非循环 gold = **po_slice 131 人工 point-label**(`po_labels_filled.csv`)。**离线前导回归已跑**(`phase5_factory/offline_human_gold_regression.json`,24 对全覆盖):新 coverage 判分对人工 **end-to-end MAE 7.2%**,分解=verdict 3.1%(AI 判分)+ arithmetic 4.7%(均权 vs 人工真实给分)。**发现:AI 判分误差<均权算术误差(瓶颈在算术非 AI);4.7% 均权 gap 是 decision#2 证据(印证选 A 先度量)。** 缺口曾为 (a) legacy 臂 +(b) 规模化人工 gold。
> **(a) legacy 臂已补(2026-06-14,身份重映射解锁)**:po_slice gold **丢了具体题号只留章节节点 + 3/12 错标节点**(`Q20-1A413000` 实为 1A435000、`Q6-1A413000` 实为 1A434000、`Q9-1A413040` 实为 1A434000——**非冻结 canonical taxonomy 出错,是 gold 的 case_id 标注错**)。content-match 官方答案恢复正确 chunk_id,**12/12 在 legacy bank**(join 表 `po_slice_identity_remap.json`)。双臂回归(`legacy_arm_regression.json`):**MAE(new)=7.2% vs MAE(legacy)=14.1% → new 误差约 legacy 一半,MAE 门方向性过**;但 **over-credit:new 2 / legacy 0**(legacy 求和漏点系统欠给、new 偶发过给)→ **over-credit-not-higher 门小样本不过**。**诚实净结论:new 整体准得多但有偶发过给失败模式(正是 `detect_over_credit` 该盯的)**。driver:`phase10/phase11`。
> **(b) 多-AI 非循环 scaled gold 已跑(2026-06-14,phase12-13,设计见 `MULTI_AI_NONCIRCULAR_GOLD_DESIGN.md`)**:用 golden_v1 的 100 对学生答卷(非其 AI gold)产 **485 点非循环 gold**——DeepSeek+Qwen 盲标→consensus 446(92%)=high→分歧/错 39 送 Opus 终裁=medium(0 contested);**门 verdict 排除生产模型 deepseek-chat**=非循环。人工验证(131 点 overlap)93.9%。**全量非循环门(零额外 LLM):NEW(生产 DeepSeek coverage)vs scaled gold(排 DeepSeek)100 对 MAE 2.4%、over-credit 3/100** → 生产判分对独立 gold 97.6% 准。**5 个错标节点已修(`83506e817`+`c066237f9`,强判别 full-content 匹配;Q4/Q8/Q10/Q19 模糊诚实 flag 不改)**。
> **scaled 双门已闭(phase14,16 非模糊案例/80 对 vs 非循环 scaled gold)**:**MAE new 2.2% ≤ legacy 10.7% ✓;over-credit new 2 ≤ legacy 7 ✓ → 双门 PASS**。**⚠️ 诚实 nuance:over-credit 半门对 gold 敏感**——对**人工 gold**(24 对,phase11)时 new 2 > legacy 0(**不过**),对**scaled gold**(80 对)时 new 2 < legacy 7(**过**),因人工标签比踩字严格的 ensemble gold 更宽松(人给 partial/hit 处 ensemble 判 miss)→legacy required_terms 反而 over-credit 严格 ensemble。判读双门须声明锚哪个 gold。
> **③ PO 分层抽样包已备(phase15)**:`scaled_gold/po_sampling_package.{json,csv}`,**114 项=24% of 485**(39 medium 全审 + 75 high 分层抽样,每 case×verdict 层≥1),真人填 `po_verdict` 即对 scaled gold 做 v1 签——人类锚省到刀刃。**仍缺**:PO 真人跑这个抽样包背书 + 真实学生流量(现答卷是合成 archetype)。产物:`scaled_gold/{scaled_gold,scaled_double_gate,po_sampling_package}.json`。
