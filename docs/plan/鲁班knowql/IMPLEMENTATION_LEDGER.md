...
| L5 signed authorization package draft | `landed-local` | `scripts/run_luban_knowql_nexus_l5_signed_authorization_package.py`; `tests/scripts/test_luban_knowql_nexus_l5_signed_authorization_package.py`; artifact `knowql_nexus_stage5_human_boundary_repair_20260615T154400Z/signed_authorization_package.json` | Generates production-default and canonical-truth authorization forms, but keeps both unsigned and no-write. Current verdict `BLOCKED_BEFORE_SIGNATURE` because Stage5 canary is not ready and `stage5_human_gold_over_credit_blocker` remains. |
