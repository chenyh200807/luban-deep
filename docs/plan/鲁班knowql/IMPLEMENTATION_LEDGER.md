...
| 15 | Ground as runtime gate | `_classify_point_ground`; `retrieve_rubric`; `_project_pgo_record`; `deep_question._summarize_pgo_query_result` | `landed-local` | 三分层落地：`_classify_point_ground` 把每个 point 判为 score_bearing(official_slice)/ supporting_only(textbook provenance，永不进对错通道)/ unsourced。scoring shape(grading/rubric_table)若 `score_bearing_count==0` 直接 fail open（`reason=scoring_shape_without_score_bearing_ground`，不依赖 citation_required、无 prompt 兜底）；投影暴露 per-point `ground_class`+`scorable`，`ground` 返回 score_bearing/supporting/unsourced 分层计数。Consumer 已真消费：`_summarize_pgo_query_result` 据 `scorable` 算 `scorable_point_count`+`has_unscorable_points`（判分分母剔除 supporting/unsourced），经 `_maybe_attach_pgo_shadow` 的本地真实 `/api/v1/ws` shadow 路径。验证：`pytest tests/services/construction_grading/test_m35_artifact_query.py tests/core/test_deep_question_case_rubric_v1.py tests/integration/test_luban_pgo_knowql_ws_readback.py -q`。诚实边界：真实 supply 1384/1384 全 score_bearing，fail-open 与 `has_unscorable_points=true` 分支仅 fixture 覆盖、未经真实流量触发。 | 远端 readback 证明真实流量真的触发 fail-open / 不可判分分支（消费侧已闭合，缺的是真实 unsourced 流量证据）。 |
