...
| L2 true-entry three-arm runner v3 prereg | `landed-local` | `scripts/run_luban_knowql_nexus_l2_learning_ab.py`; `tests/scripts/test_luban_knowql_nexus_l2_learning_ab.py`; local tests lock 5 PGO-backed scenarios, `sample_manifest_hash`, prereg primary metric `b2_outcome_miss_reduction_lift_vs_b1`, and B3 guardrails | Formal L2 live run is pending. B2 wiring/readback concern is no longer the next blocker; one-loop live artifact `knowql_nexus_l2_learning_ab_20260615T122930Z` showed B2 PGO/KnowQL/G3 readback 2/2 and safety GO, but effect neutral. |
...
- Formal powered live evidence remains pending until a larger paced test2/cohort run completes with prereg `loops>=10`, `min_loops>=10`, 5 fixed PGO-backed scenarios, and `canonical_truth_written=false` / `official_score_written=false`.
...
Interpretation: this proves the new B2 integrated Nexus/KnowQL/GBrain/NBA loop can be observed through the real test2 entry with write safety intact. It still does not prove product-level learning efficiency. The next true-entry run must be the preregistered larger L2 run, not another B2 wiring smoke.
