...
| L2 true-entry three-arm runner v2 | `live-readback` | `scripts/run_luban_knowql_nexus_l2_learning_ab.py`; `tests/scripts/test_luban_knowql_nexus_l2_learning_ab.py`; live artifact `knowql_nexus_l2_learning_ab_20260615T114120Z` | Runner works through true `/api/v1/ws`, but decision is NO-GO: test2 did not emit B2 PGO shadow / KnowQL / PGO-G3 readback. A0 also observed V1 score-first default, so pure RAG/ref baseline is not isolated on current test2. |
...
- `raw_learning_rows.jsonl` contains A0/B1/B2 initial/retest WS rows, score-ratio deltas, PGO miss deltas, TTFT, first-result latency, streaming, sealed-block status, score-first observation, payload, and safety metadata.
...
- `summary.json` separates `safety_status` from `effect_status`; any canonical truth write, official score write, A0 PGO contamination, B1 PGO contamination, or B2 KnowQL/G3 missing is `L2_SAFETY_NO_GO`.
- Formal powered live evidence remains pending until a larger paced test2/cohort run completes under the v2 arm mapping with qa/operator cohort identities and `canonical_truth_written=false` / `official_score_written=false`; the 2026-06-15 evidence below is one-loop smoke only under the older arm mapping.
...
Interpretation: this proves the old L2 evidence chain could be exercised through the real test2 entry and could observe a PGO point-level retest delta. It does not prove product-level learning efficiency for real learners, and it does not prove the new v2 arm mapping. Next true-entry run must use A0 baseline / B1 Nexus V1 without KnowQL / B2 Nexus V1 + KnowQL/GBrain/NBA.
