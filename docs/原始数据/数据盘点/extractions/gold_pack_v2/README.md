...
| 题 | 采分点数 | 满分（分母） | Σ点分（点池） | high | mid | low |
|---|---|---|---|---|---|---|
| Q2023-03 | 19 | 20.0 | 20.0 | 16.8 (0.840) | 12.5 (0.625) | 4.20 (0.210) |
| Q2024-03 | 11 | **20.0** | **22.0** ⚠ | 19.0 (0.950) | 15.0 (0.750) | 6.00 (0.300) |
| Q2025-03 | 13 | 20.0 | 20.0 | 18.0 (0.900) | 13.5 (0.675) | 3.83 (0.192) |

`expected_point_score` 由 expected_failures 推导（踩点累加、不封顶），**不是**独立标注的分数，
只作一致性自检信号：三题的 high > mid > low 单调，且档位间隔明显（≈0.93 / ≈0.68 / ≈0.23）。

> **⚠ Q2024-03 满分订正（2026-08-01，canonical431 Lane 1 §3.4）。**
> 原先分母写 22.0 = Σ佑森逐点分值。佑森是本 pack 唯一的分值来源，自己校自己等于
> 没校。Lane 1 引入了唯一一条**不来自佑森**的外部锚 —— 一建·建筑实务卷面结构
> （案例一~三各 20 分、案例四~五各 30 分，合计 120），用它压 20 个案例组：**18 组
> 精确吻合，2 组走样**，其中 `2024-case3`（= 本题）Σ = 22.0，比卷面多 2.0。
> 金标继承了同一处走样，**任何用它算满分率的地方分母都偏大 10%**。
>
> 订正口径：`rubric_total_score` 改用卷面锚 **20.0**（`rubric_total_score_authority
> = official_paper_structure_1a_jianzhu`），Σ点分另存 `rubric_point_pool_total = 22.0`，
> 并在题级打 `nominal_authority_disputed: true` + `nominal_dispute_detail`。
> **per-小问的 `sub_q_total_score` 一个字没动**（仍是 5/4/4/5/4，Σ=22）：哪一问多了
> 2.0 是未裁决的，编一个分配方案会把「不知道」伪装成「知道」，比留着不一致更危险。
> 副作用要知悉：本题**全中会算出 110%**（22/20）—— 这正是走样本身，不是计算错误。
>
> 上游同源：canonical431 bank 对 `2024-case3` / `2025-case5` 的每条记录也打了
> `nominal_authority_disputed`，判分核在 tier-1 封顶链上对这两组 **fail-closed 拒用
> 分母**（走样的满分去封顶比不封顶更危险）。两侧用同一套词汇，不是各写各的。
