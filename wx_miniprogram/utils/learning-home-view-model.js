...
var HOME_PROJECTION_TOPIC_AUTHORITY = "learner_state.home_personalization.canonical_taxonomy";

function homeProjectionSourceStatus(body) {
  var projection = asObject(body.home_projection);
  return asObject(projection.source_status || body.source_status);
}

function hasHomeProjectionSurface(body) {
  var payload = asObject(body);
  if (payload.home_projection && typeof payload.home_projection === "object") return true;
  if (asList(payload.recommended_prompts).length) return true;
  var today = asObject(payload.today);
  var focus = asObject(payload.today_focus || today.focus);
  var meta = compactText(focus.meta || "");
  return /learner_state\.home_personalization|学情更新/.test(meta);
}
