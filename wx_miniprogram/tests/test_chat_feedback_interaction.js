// test_chat_feedback_interaction.js — feedback popover should be selectable and submit reliably
// Run: node wx_miniprogram/tests/test_chat_feedback_interaction.js
...
async function run() {
  ...
  assert(
    ...
    "feedback selected state should use a WXS reader that mini-program templates can execute",
  );
  assert(
    wxml.indexOf("feedbackTags.indexOf(") === -1,
    "feedback selected state should not rely on indexOf calls inside WXML expressions",
  );
  ["事实错误", "产生幻觉", "逻辑不通", "格式混乱"].forEach(function (tag) {
    assert(
      wxml.indexOf("feedbackTag.has(feedbackTags, '" + tag + "')") >= 0,
      "feedback tag should render selected class from canonical feedbackTags: " + tag,
    );
  });
...
run().then(function () {
  ...
  console.log("PASS test_chat_feedback_interaction.js (" + pass + " assertions)");
});
