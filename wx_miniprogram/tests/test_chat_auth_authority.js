...
function loadChatPage(overrides) {
  ...
  var apiMock = Object.assign(
    {
      ...
      getUserInfo: function () {
        return Promise.resolve({ id: "wx_user_bootstrap", username: "chenyh2008", points: 18 });
      },
    },
  ...
  var sandbox = {
    ...
    require: function (request) {
      ...
      if (request === "../../utils/surface-telemetry") return { track: function () {} };
...
(async function main() {
  await run("wx chat bootstrap should not rewrite auth authority from auth profile payload", async function () {
    ...
    assert(
      ...
      "wx chat bootstrap must not call auth.setToken after getUserInfo succeeds",
    );
  });
