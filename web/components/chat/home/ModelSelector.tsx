...
export default function ModelSelector({
  ...
  const selectedSelection = value ?? activeDefault;
  ...
  const label = loading
    ...
    : error
      ...
      : selectedOption?.model_name || t("Model");
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && !disabled && (
        <div className="absolute bottom-full right-0 z-50 mb-1.5 w-[min(340px,calc(100vw-32px))] overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--popover)] shadow-lg backdrop-blur-md">
          <div className="border-b border-[var(--border)]/50 px-3 py-2">
            ...
            <div className="mt-0.5 text-[10px] text-[var(--muted-foreground)]">
              {t("Applies to the next message in this chat")}
            </div>
          </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 items-center gap-1.5">
                      ...
                      {option.is_active_default && (
                        <span className="shrink-0 rounded-full bg-[var(--muted)] px-1.5 py-px text-[9px] font-semibold uppercase text-[var(--muted-foreground)]">
                          {t("Default")}
                        </span>
                      )}
                    </div>
