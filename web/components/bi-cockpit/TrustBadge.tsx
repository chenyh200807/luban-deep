...
/**
 * 独立可信度等级胶囊（设计板 KPI 卡右上角形态）：只渲染「● X 级」+ hover 元数据卡，
 * src chip / 微条由调用方按设计板布局自行排布。
 */
export function TrustPill({ metricId }: { metricId: string }) {
  const meta = findMetricById(metricId)
  const color = TRUST_LEVEL_COLORS[meta.trust]
  return (
    <span className="group/trust relative inline-flex">
      <span
      ...
      <TrustTooltip meta={meta} color={color} />
    </span>
  )
}
