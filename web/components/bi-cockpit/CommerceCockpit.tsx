...
export function CommerceCockpit({ data }: { data: BiCommerceData | null }) {
  ...
  const pkgPoints: Datum[] = top(
    packages.map(p => ({ name: p.name, value: num(p.points) })).filter(x => x.value > 0),
    6
  )
    ...
    <CockpitBg className="p-4 md:p-5">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-7">
        ...
        <CockpitKpi
          ...
          tone={num(s?.anomalyCount) > 0 ? 'rose' : 'emerald'}
        ...
        >
          {creditDebit.length ? (
            <CockpitDonut
              data={creditDebit}
              centerLabel="净"
              centerValue={fmt(num(s?.creditPoints) - num(s?.debitPoints))}
            />
          ) : (
            <Empty />
          )}
        ...
        <CockpitPanel title="账本类型构成" icon={<PieChart className="h-4 w-4" />}>
          {ledgerKind.length ? (
            <CockpitDonut
              data={ledgerKind}
              centerLabel="流水"
              centerValue={fmt(num(s?.ledgerCount))}
            />
          ) : (
            <Empty />
          )}
        ...
        <CockpitPanel title="充值渠道" icon={<PieChart className="h-4 w-4" />}>
          {channel.length ? (
            <CockpitDonut
              data={channel}
              centerLabel="充值"
              centerValue={fmt(num(s?.rechargeCount))}
            />
          ) : (
            <Empty />
          )}
          ...
          {anomalies.length ? (
            <ul className="space-y-2">
              {anomalies.slice(0, 6).map((a, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-2.5 text-[12px]"
                >
                  <span
                    className="mt-1 h-2 w-2 shrink-0 rounded-full"
                    style={{ background: SEV_COLOR[a.severity] ?? SEMANTIC.neutral }}
                  />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-bold text-slate-100">
                      {a.description || a.ruleId}
                    </span>
                    <span className="mt-0.5 block truncate text-[11px] text-slate-400">
                      影响 {fmt(num(a.affected))} · {a.owner || '—'} · {a.status || '—'}
                    </span>
                  </span>
                </li>
              ))}
            ...
            <div className="grid h-[200px] place-items-center rounded-xl border border-dashed border-emerald-300/15 text-[12px] text-emerald-200/80">
              无账务异常 ✓
            </div>
          )}
        </CockpitPanel>
