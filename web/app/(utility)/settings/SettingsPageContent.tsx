...
function SettingsPageContent() {
  ...
  const searchProviderRaw =
    activeService === 'search' ? (activeProfile?.provider || '').trim().toLowerCase() : ''
  ...
  const runSingleTest = useCallback(
    async (svc: ServiceName, catalogSnapshot: Catalog): Promise<'pass' | 'fail'> => {
      try {
  ...
  const reopenTour = async () => {
    ...
    setToast(
      payload.command
        ...
        : payload.message || t('Run python scripts/start_tour.py in your terminal.')
    )
  }
        ...
        <div className="mb-8 flex flex-wrap items-center gap-x-8 gap-y-3 border-b border-[var(--border)]/50 pb-6">
          ...
          <div className="ml-auto flex items-center gap-4 text-[12px] text-[var(--muted-foreground)]">
            ...
            <span className="flex items-center gap-1.5">
              ...
              {t('LLM')}
              {status?.llm.model && (
                <span className="text-[var(--muted-foreground)]/50">· {status.llm.model}</span>
              )}
            </span>
            <span className="flex items-center gap-1.5">
              <span
                        ...
                        {activeService === 'embedding' && (
                          <div>
                            ...
                            <input
                              ...
                              onChange={e => updateModelField('dimension', e.target.value)}
                            />
                          </div>
                        )}
                      </div>
