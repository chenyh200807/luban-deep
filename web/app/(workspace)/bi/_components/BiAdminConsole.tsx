    ...
    <span className="flex flex-wrap gap-1">
      {tabs.map(tab => (
        <span
          ...
          style={{ background: 'rgba(28,19,13,0.6)', color: COCKPIT.textMuted, border: `1px solid ${COCKPIT.grid}` }}
        >
          {labelOf(tab)}
        </span>
      ))}
  ...
  return (
    <CockpitBg className="p-4 md:p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em]" style={{ color: alpha(COCKPIT.accentBright, 0.9) }}>
        ...
        {!canManage && !loading ? (
          <span
            ...
            style={{ background: 'rgba(110,95,82,0.22)', color: COCKPIT.textMuted, border: `1px solid ${COCKPIT.grid}` }}
          >
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        />
        <CockpitKpi label="管理员总数" value={admins.length} tone="violet" icon={<Users className="h-4 w-4" />} />
      ...
      {error ? (
        <div
          ...
          style={{ background: alpha(DANGER, 0.12), color: DANGER, border: `1px solid ${alpha(DANGER, 0.35)}` }}
        >
          ...
          {picked ? (
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  style={{ background: 'rgba(20,14,10,0.7)', border: `1px solid ${alpha(SERIES_COLORS[0], 0.3)}`, color: COCKPIT.text }}
                  aria-label="选择角色"
                >
            ...
            <div className="relative">
              ...
              <input
                ...
                style={{ background: 'rgba(20,14,10,0.7)', border: `1px solid ${alpha(SERIES_COLORS[0], 0.25)}`, color: COCKPIT.text }}
                aria-label="搜索会员"
              />
              {query.trim() ? (
                <div
                  className="absolute z-20 mt-1.5 w-full overflow-hidden rounded-xl shadow-2xl"
                  style={{ background: COCKPIT.bgPanelSolid, border: `1px solid ${alpha(SERIES_COLORS[0], 0.3)}` }}
                >
                  {searching ? (
                          ...
                          >
                            <span className="flex items-center gap-3">
                              <span
                                ...
                                style={{ background: alpha(SERIES_COLORS[0], 0.16), color: COCKPIT.accentBright }}
                              >
                                {(m.display_name || m.user_id).slice(0, 1)}
                              </span>
                              <span className="min-w-0">
                                <span className="block truncate text-[13px]" style={{ color: COCKPIT.text }}>
                            ...
                            {m.current_role ? (
                              <RoleBadge
                                role={m.current_role}
                                label={assignableRoles.find(r => r.key === m.current_role)?.label ?? m.current_role}
                              />
                            ) : (
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  <div
                    ...
                    style={{ background: roleBadgeStyle(a.role).bg, color: roleBadgeStyle(a.role).fg }}
                  >
                    {(a.display_name || a.user_id).slice(0, 1)}
                  </div>
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="truncate text-[13px] font-semibold" style={{ color: COCKPIT.text }}>
                        {a.display_name || '未命名'}
                      </span>
                      ...
                      <span
                        className="rounded px-1.5 py-0.5 text-[10px]"
                        style={{
                          background: a.source === 'env' ? alpha(SERIES_COLORS[0], 0.16) : 'rgba(110,95,82,0.2)',
                    ...
                    <p className="mt-1 truncate font-mono text-[11px] text-slate-500">{a.user_id}</p>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  {rowBusy === a.user_id ? <Loader2 className="h-3.5 w-3.5 animate-spin text-slate-400" /> : null}
                    ...
                    confirmRemove === a.user_id ? (
                      <span className="flex items-center gap-1.5">
                        <button
                          ...
                          style={{ background: alpha(DANGER, 0.16), color: DANGER, border: `1px solid ${alpha(DANGER, 0.4)}` }}
                        >
                          确认移除
                        </button>
                        <button
      ...
      <CockpitPanel className="mb-4" hint="每个角色在各分区可执行的操作一览（查看 / 导出 / 写入 / 高危）">
        {roles ? (
                ...
                {audit.map((entry, idx) => (
                  <li key={`${entry.ts}-${entry.target}-${idx}`} className="relative">
                    <span
                      ...
                      style={{ background: SERIES_COLORS[0], boxShadow: `0 0 8px ${COCKPIT.borderGlow}` }}
                    />
...
/** 角色 × tab × action 可视化矩阵：行=角色，列=tab，单元格内 action 勾选。 */
function RoleMatrix({ roles }: { roles: BiRbacRoles }) {
      ...
      <table className="w-full border-collapse text-[11px]">
        <thead>
          <tr>
            ...
            {roles.tabs.map(tab => (
              <th key={tab.key} className="px-2 py-2 text-center font-bold" style={{ color: COCKPIT.textMuted }}>
                {tab.label}
              </th>
            ))}
          </tr>
          ...
          {roles.roles.map(role => (
            <tr key={role.key} style={{ borderTop: `1px solid ${COCKPIT.grid}` }}>
              ...
              >
                ...
                <p className="mt-1 max-w-[140px] text-[10px] leading-4 text-slate-500">{role.description}</p>
              </td>
              {roles.tabs.map(tab => {
                const granted = role.matrix[tab.key] ?? []
                return (
                    ...
                    {granted.length === 0 ? (
                      ...
                      <span className="flex flex-wrap justify-center gap-1">
                        {roles.actions.map(action => {
                          ...
                          const high = action.key === 'high_risk'
                          return (
                            <span
                              key={action.key}
                              title={`${actionLabelOf(action.key)}${on ? '：允许' : '：无'}`}
                              ...
                              style={{
                                ...
                                border: `1px solid ${on ? (high ? alpha(SEMANTIC.danger, 0.4) : alpha(SERIES_COLORS[0], 0.35)) : 'transparent'}`,
                              }}
                            >
                              {on ? <Check className="h-2.5 w-2.5" /> : <X className="h-2.5 w-2.5" />}
                              {actionLabelOf(action.key)}
                            </span>
                          )
                        })}
                      </span>
