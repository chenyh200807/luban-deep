...
import { BiCostOfficialAnchorCard } from './BiCostOfficialAnchorCard'
import {
  InfoLine,
  MetricCard,
  MiniStatCard,
...
export function BiTutorBotTab({
  ...
  return (
    <>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        {metricCards.length ? (
          ...
          <article className="surface-card col-span-full border border-[var(--border)]/60 bg-white/88 p-6 shadow-[0_14px_32px_rgba(45,33,25,0.06)]">
            <p className="text-xs tracking-[0.22em] text-[var(--muted-foreground)]">
              TUTORBOT WORKBENCH
            </p>
            <h2 className="mt-3 text-2xl font-semibold tracking-tight text-[var(--foreground)]">
              TutorBot 指标补齐后，这里会承接运行主视图
            </h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--secondary-foreground)]">
              当前还没有返回 TutorBot
              指标卡片，但下面仍保留排行、状态、最近活跃、最近消息和知识联动位。接口补齐后会自动落到这条运行视图，不需要再回首页找。
            </p>
          </article>
        )}
      </section>
        ...
        <div className="space-y-6">
          <div className="surface-card p-5">
                ...
                <SectionHeader
                  ...
                  extra={
                    tutorbots.statusBreakdown.length
                      ? `${tutorbots.statusBreakdown.length} 个状态切片`
                      : `${days} 天窗口`
                  }
                />
                <p className="mt-3 text-sm leading-6 text-[var(--secondary-foreground)]">
                  用同一屏同时看运行指标、状态分布和近期活跃，避免 TutorBot 数据继续埋在 Overview
                  里。
                </p>
              </div>
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
                  ...
                  tutorbots.statusBreakdown
                    ...
                    .map(item => (
                      <MiniStatCard
                        ...
                        value={item.value}
                        hint={
                          item.rate !== undefined ? `占比 ${formatPercent(item.rate)}` : item.hint
                        }
                      />
                    ))
                ) : (
              ...
              <div className="rounded-[28px] border border-[var(--border)]/60 bg-[linear-gradient(180deg,rgba(195,90,44,0.08),rgba(255,255,255,0.86))] p-5">
                <p className="text-xs tracking-[0.2em] text-[var(--muted-foreground)]">
                  WATCH LIST
                </p>
                    ...
                    <div className="rounded-2xl border border-[var(--border)]/60 bg-white/70 px-4 py-4">
                      ...
                      <div className="mt-3 space-y-2">
                        {highlightItems.map(item => (
                          <p
                            key={item}
                            className="text-sm leading-6 text-[var(--secondary-foreground)]"
                          >
                            {item}
                          </p>
                        ))}
          ...
          <div className="surface-card p-5">
            <SectionHeader
              title="最近活跃"
              extra={
                activitySamples.length ? `${activitySamples.length} 个样本` : '当前没有活跃样本'
              }
            />
            <div className="mt-4 space-y-3">
              {activitySamples.length ? (
                activitySamples.map(bot => (
                  <TutorBotActiveCard key={bot.bot_id || bot.name} bot={bot} />
                ))
              ) : (
                <p className="rounded-2xl bg-[var(--secondary)] px-4 py-4 text-sm text-[var(--muted-foreground)]">
                  当前没有最近活跃样本，说明这段时间没有可展示的 TutorBot
                  活动，不等同于数据加载失败。
                </p>
              )}
            </div>
        ...
        <aside className="space-y-6">
            ...
            <div className="mt-4 space-y-3">
              {messageSamples.length ? (
                messageSamples.map(bot => (
                  <TutorBotMessageCard
                    key={bot.bot_id || bot.name || bot.recent_message}
                    bot={bot}
                  />
                ))
              ) : (
          ...
          <div className="surface-card p-5">
            <div className="flex items-start justify-between gap-3">
              <SectionHeader
                title="知识库副面板"
                extra={knowledge.items.length ? `${knowledge.items.length} 个知识源` : '副面板'}
              />
            ...
            <div className="mt-4 space-y-3">
              <InfoLine
                label="零结果率"
                value={
                  knowledge.zeroHitRate !== undefined
                    ? formatPercent(knowledge.zeroHitRate)
                    : '暂无'
                }
              />
              <InfoLine
            ...
            <div className="mt-4">
              <SimpleListCard
                ...
                footer={
                  knowledge.topQueries.length
                    ? `热查询 ${knowledge.topQueries.length} 条`
                    : undefined
                }
              />
            </div>
        ...
        <div className="surface-card p-5">
          ...
          <div className="mt-4 space-y-3 text-sm leading-6 text-[var(--secondary-foreground)]">
            <InfoLine
              label="主观察"
              value={
                tutorbots.ranking.length
                  ? '优先对比高运行量 Bot 的成功率与状态分布是否同步恶化'
                  : '等待排行样本后给出主观察'
              }
            />
            <InfoLine
              label="能力侧"
              value={
                capabilities.upgradePaths.length
                  ? '把高调用 TutorBot 优先映射到升级路径最清晰的 capability'
                  : '等待升级路径返回'
              }
            />
            <InfoLine
              label="工具侧"
              value={
                tools.efficiency.length
                  ? '工具效率波动时，回看最近消息与知识命中是否同时变差'
                  : '等待工具效率返回'
              }
            />
            <InfoLine
      ...
      <BiCostOfficialAnchorCard days={days} cost={cost} />
...
function TutorBotActiveCard({ bot }: { bot: BiTutorBotItem }) {
  ...
  return (
    <div className="rounded-2xl border bg-[var(--background)] px-4 py-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-medium text-[var(--foreground)]">
            {bot.name || bot.bot_id || 'TutorBot'}
          </p>
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">
            {bot.bot_id || '--'} ·{' '}
            {bot.last_active_at ? formatTime(bot.last_active_at) : '最近活跃暂无时间戳'}
          </p>
        </div>
        {bot.status ? (
          <span className={`muted-chip ${toneClasses(tone)}`}>{bot.status}</span>
        ) : null}
      </div>
      ...
      {bot.detail ? (
        <p className="mt-3 text-sm leading-6 text-[var(--secondary-foreground)]">{bot.detail}</p>
      ) : null}
    </div>
  )
}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs text-[var(--muted-foreground)]">
            {bot.capability || 'capability 暂无'} · {bot.entrypoint || 'entrypoint 暂无'} ·{' '}
            {bot.tier || 'tier 暂无'}
          </p>
        </div>
