...
async function getDatabaseUrl() {
  return process.env.FEEDBACK_DATABASE_URL || process.env.SUPABASE_DB_URL || process.env.DB_URL || "";
}
...
function validatePayload(payload: FeedbackPayload) {
  const answers = (payload.answers && typeof payload.answers === "object" ? payload.answers : null) as
  ...
  const contact = (answers.contact && typeof answers.contact === "object" ? answers.contact : {}) as Record<
    string,
    unknown
  >;
