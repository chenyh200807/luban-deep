...
function compareFinalAndHistory(
  ...
  if (JSON.stringify(finalSummary.renderBlockTypes) !== JSON.stringify(historySummary.renderBlockTypes)) {
    warnings.push("render block types differ between final stream and history hydrate");
  }
  if (JSON.stringify(finalSummary.visibleBlockTypes) !== JSON.stringify(historySummary.visibleBlockTypes)) {
...
export function loadWechatHarnessCases(): WechatHarnessCase[] {
  ...
  return [...structuredCases, ...markdownCases, ...operationalCases];
}
