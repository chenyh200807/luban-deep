import { apiUrl, withAdminAuthorization } from './api.ts'
...
export interface ReversibleMembershipPurchase {
  purchase_id: string
  package_id: string
  amount_cny: number
  points: number
...
export interface MemberDetail {
  ...
  membership_billing?: {
    reversible_supreme_purchase?: ReversibleMembershipPurchase | null
  }
}
...
export async function recordMemberConversationView(
  ...
  // Plan §3.5 / §6 require reason on every full-text view. Backend currently
...
export async function revokeMembership(payload: {
  user_id: string
  reason?: string
}): Promise<MemberDetail> {
