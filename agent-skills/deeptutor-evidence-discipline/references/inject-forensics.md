...
【嗅觉 — 取证前扫这几条】
- 探针里带了参数,却拿输出当「生产值」 → 观测工具污染观测结果
- 用 `grep 有没有写 X` 推断运行时 X 的值 → 跳过默认值层,实测一次
- `threading.Lock` 出现在多进程服务 → 跨不过进程边界。先查 `UVICORN_WORKERS`
- 全量读写一个会随用户增长的文件 → 成本随规模爆炸,且常在锁内
...
【可测性纪律 — 取证前先问「这个问题用这个源答得出来吗」】
- **不能用一个被污染的数据源,测量它自己的污染量**。要查"历史丢了多少",
  查那个丢东西的文件是结构上无效的——丢掉的记录不在里面。
- 跨源比对必须先确认**两侧能按同一个键对齐**,且分母定义相同。
  对不齐就写「无法回溯判定」,不要拿两个数相减(那是分母混淆)。
- 回溯不了 → 明确转**前向验证**:提出一个从现在起可测的对账口径,
  而不是硬给一个不可信的历史数字。

【tripwire — 命中即停并报告】
- 你正准备用一个"恰好能连上"的面替代取不到的主证
- 你发现同一事实在两个源里数字不一致(报告不一致本身,不要择一)
- **两个权威给出矛盾答案**——先查双方各自的证据级别,常常是"都只有旁证"
- 你需要写入或修改才能取到证据
- 你要回答的问题,用手上这个源**结构上答不出来**
```
