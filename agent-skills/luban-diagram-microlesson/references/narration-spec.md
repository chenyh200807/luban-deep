...
| 模型 | 用途 | 文本怎么来 | 实现 |
|---|---|---|---|
| **纯派生(本总纲)** | 单卡旁白播放器(F16/C01…) | 固定模板插值卡字段,**零创作** | `build_card_narration.mjs::deriveNarration` |
| **作者撰写 + anchor 闸** | 教学动画/双人对话(讲懂幕) | 讲解口吻/类比/PPT **是创作的**,但每个 `claim:true` 段**必须 anchor 回卡真实字段**,配音前校验,解析不到报错 | `build_lesson_narration.mjs` + [[teaching-animation-journey]] |

纯派生保证"绝不漂移但口吻死板";教学动画要"有趣拉人"就得允许创作,于是用 **anchor 防漂移闸**兜底事实层(口语黏合标 `claim:false` 不含事实)。**两者都做到"考点 traceable 回卡",只是一个靠模板、一个靠闸。** 下文是纯派生模型的总纲。
