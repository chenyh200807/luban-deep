...
- **现状**:✅ 草稿 `artifacts/luban_case_family_assets/diagram_microlesson/C01_construction_joint_contrast.schema_draft.json`(施工缝留置;`contrast_pair_reveal_draft`)已有 prototype renderer `render_contrast_card.py`,但仍是 draft/candidate,不是生产模板。它是 panel 加固的参考实现。可与 ⑥诊断共用错因/采分点资产;接生产前继续硬化:template_type 分派 + body 互斥 + student-safe 白名单 + preview/student 包扫描。
