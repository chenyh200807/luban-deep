...
| 幻灯片 | Slide Scene + React Renderer + PPTX Export | P0 | 必须做 |
| 测验 | Question Engine | P0 | 必须做 |
| 案例题批改 | Rubric Grader + Weakness Tracker | P0 | 这是我们超过通用课堂的核心 |
| 交互仿真 | Lightweight Simulation Renderer | P1 | P0 做轻交互 |
| PBL | PBL Scenario Engine | P1 | P0 做结构化案例课 |
| 白板 | SVG Whiteboard + Action Timeline | P0 | P1 增强动画 |
| TTS | TTS Provider Adapter | P0 | P0 先 scene 级别音频 |
...
| Realtime | SSE + WebSocket | SSE 推生成进度，WebSocket 做课堂实时问答 |
...
| 层级 | 技术 | 选择理由 |
|---|---|---|
| 主框架 | Next.js / React / TypeScript | 与 DeepTutor 和 OpenMAIC 都相近 |
| 样式 | Tailwind CSS | 快速构建一致 UI |
| 状态管理 | Zustand | 播放器状态、scene 状态轻量好用 |
| 数据校验 | Zod | 前后端 schema 对齐 |
| 白板 | SVG 优先，Canvas 后置 | SVG 更利于导出、调试、PPT 静态化 |
| 幻灯片 | React Scene Renderer | 同一组件用于播放和 HTML 导出 |
| 交互仿真 | React Components | 每个仿真是独立 scene component |
| 播放器 | 自研 ClassroomPlayer | 使用 Action Timeline 驱动 |
...
┌──────────────────────────────────────────────────────────┐
│                        Web Frontend                       │
│  New Course │ Outline Editor │ Player │ Review │ Export   │
└───────────────┬──────────────────────────────────────────┘
                │ HTTP / SSE / WebSocket
┌───────────────▼──────────────────────────────────────────┐
│                    Exam Classroom API                     │
...
```text
SimulationIR → React Simulation Component → 用户行为 → 判分规则 → 反馈
```
